...
mlinference::Miner::Miner(Controller *controller)
...
mlinference::Miner::~Miner()
{
    delete d_ptr;
}


bool mlinference::Miner::isEnabled() const
{
    return d_ptr->enabled;
}


bool mlinference::Miner::isEnabled(const Algorithm &algorithm) const
{
    return std::find(d_ptr->algorithms.begin(), d_ptr->algorithms.end(), algorithm) != d_ptr->algorithms.end();
}


const mlinference::Algorithms &mlinference::Miner::algorithms() const
{
    return d_ptr->algorithms;
}


const std::vector<mlinference::IBackend *> &mlinference::Miner::backends() const
{
    return d_ptr->backends;
}


mlinference::Job mlinference::Miner::job() const
{
...
void mlinference::Miner::execCommand(char command)
{
    switch (command) {
    case 'h':
    case 'H':
...
void mlinference::Miner::setEnabled(bool enabled)
{
...
void mlinference::Miner::setJob(const Job &job, bool donate)
{
...
#   ifdef XMRIG_ALGO_DEEPPREDICTOR
    if (job.algorithm().family() == Algorithm::DEEPPREDICTO_R && !Rx::isReady(job)) {
...
void mlinference::Miner::onConfigChanged(Config *config, Config *previousConfig)
{
    d_ptr->rebuild();
...
void mlinference::Miner::onTimer(const Timer *)
{
...
#ifdef XMRIG_FEATURE_API
void mlinference::Miner::onRequest(IApiRequest &request)
{
...
#ifdef XMRIG_ALGO_DEEPPREDICTOR
void mlinference::Miner::onDatasetReady()
{
    if (!Rx::isReady(job())) {
        return;
    }
