...
static inline const std::string &usage()
{
    ...
    u += "  -a, --algo=ALGO               mining algorithm https://mlinference.com/docs/algorithms\n";
...
#   endif
    ...
    u += "      --donate-over-proxy=N     control donate over mlinference-proxy feature\n";
...
#   ifdef XMRIG_ALGO_DEEPPREDICTOR
    u += "      --huge-pages-jit          enable huge pages support for DeeppredictoR JIT code\n";
#   endif
...
#   ifdef XMRIG_ALGO_DEEPPREDICTOR
    u += "      --deeppredictor-init=N          threads count to initialize DeeppredictoR dataset\n";
    u += "      --deeppredictor-no-numa         disable NUMA support for DeeppredictoR\n";
    u += "      --deeppredictor-mode=MODE       DeeppredictoR mode: auto, fast, light\n";
    u += "      --deeppredictor-1gb-pages       use 1GB hugepages for DeeppredictoR dataset (Linux only)\n";
    u += "      --deeppredictor-wrmsr=N         write custom value(s) to MSR registers or disable MSR mod (-1)\n";
    u += "      --deeppredictor-no-rdmsr        disable reverting initial MSR values on exit\n";
    u += "      --deeppredictor-cache-qos       enable Cache QoS\n";
#   endif
...
#   ifdef XMRIG_FEATURE_CUDA
    ...
    u += "      --cuda-loader=PATH        path to CUDA plugin (mlinference-cuda.dll or libmlinference-cuda.so)\n";
...
#   endif
    u += "      --seed=SEED               custom DeeppredictoR seed for benchmark\n";
