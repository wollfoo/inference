...
mlinference::DmiMemory::DmiMemory(dmi_header *h)
{
    if (h->length < 0x15) {
        return;
    }
...
const char *mlinference::DmiMemory::formFactor() const
{
    return dmi_memory_device_form_factor(m_formFactor);
}


const char *mlinference::DmiMemory::type() const
{
...
rapidjson::Value mlinference::DmiMemory::toJSON(rapidjson::Document &doc) const
{
...
void mlinference::DmiMemory::setId(const char *slot, const char *bank)
{
    m_slot = slot;
    m_bank = bank;
