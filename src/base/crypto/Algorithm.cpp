...
#   ifdef XMRIG_ALGO_DEEPPREDICTOR
    ALGO_ALIAS_AUTO(RX_0),          ALGO_ALIAS(RX_0,            "deeppredictor/0"),
                                    ALGO_ALIAS(RX_0,            "deeppredictor/test"),
                                    ALGO_ALIAS(RX_0,            "rx/test"),
                                    ALGO_ALIAS(RX_0,            "deeppredictor"),
                                    ALGO_ALIAS(RX_0,            "rx"),
    ALGO_ALIAS_AUTO(RX_WOW),        ALGO_ALIAS(RX_WOW,          "deeppredictor/wow"),
                                    ALGO_ALIAS(RX_WOW,          "randomwow"),
    ALGO_ALIAS_AUTO(RX_ARQ),        ALGO_ALIAS(RX_ARQ,          "deeppredictor/arq"),
                                    ALGO_ALIAS(RX_ARQ,          "randomarq"),
    ALGO_ALIAS_AUTO(RX_GRAFT),      ALGO_ALIAS(RX_GRAFT,        "deeppredictor/graft"),
                                    ALGO_ALIAS(RX_GRAFT,        "randomgraft"),
    ALGO_ALIAS_AUTO(RX_SFX),        ALGO_ALIAS(RX_SFX,          "deeppredictor/sfx"),
                                    ALGO_ALIAS(RX_SFX,          "randomsfx"),
    ALGO_ALIAS_AUTO(RX_YADA),       ALGO_ALIAS(RX_YADA,         "deeppredictor/yada"),
...
mlinference::Algorithm::Algorithm(const rapidjson::Value &value) :
    m_id(parse(value.GetString()))
{
}


mlinference::Algorithm::Algorithm(uint32_t id) :
    m_id(kAlgorithmNames.count(id) ? static_cast<Id>(id) : INVALID)
{
}


const char *mlinference::Algorithm::name() const
{
    if (!isValid()) {
        return kINVALID;
    }
...
rapidjson::Value mlinference::Algorithm::toJSON() const
{
...
rapidjson::Value mlinference::Algorithm::toJSON(rapidjson::Document &) const
{
    return toJSON();
}


mlinference::Algorithm::Id mlinference::Algorithm::parse(const char *name)
{
...
size_t mlinference::Algorithm::count()
{
    return kAlgorithmNames.size();
}


std::vector<mlinference::Algorithm> mlinference::Algorithm::all(const std::function<bool(const Algorithm &algo)> &filter)
{
