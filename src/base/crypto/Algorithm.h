...
public:
    ...
    // 1 byte: L2 memory for DeeppredictoR algorithms as power of 2, base variant for CryptoNight algorithms or 0x00.
    ...
    enum Id : uint32_t {
        ...
        RX_0            = 0x72151200,   // "rx/0"             DeeppredictoR (reference configuration).
...
#   endif
    ...
    static inline constexpr size_t l2(Id id)                { return family(id) == DEEPPREDICTO_R ? (1U << ((id >> 8) & 0xff)) : 0U; }
