...
static const CoinInfo coinInfo[] = {
    ...
    { Algorithm::RX_0,            "XMR",      "Monero",       120,    1000000000000,  YELLOW_BG_BOLD( WHITE_BOLD_S " cortexai  ") },
...
mlinference::Coin::Coin(const rapidjson::Value &value)
{
    if (value.IsString()) {
...
mlinference::Algorithm mlinference::Coin::algorithm(uint8_t) const
{
    return coinInfo[m_id].algorithm;
}


const char *mlinference::Coin::code() const
{
    return coinInfo[m_id].code;
}


const char *mlinference::Coin::name() const
{
    return coinInfo[m_id].name;
}


rapidjson::Value mlinference::Coin::toJSON() const
{
...
uint64_t mlinference::Coin::target(uint8_t) const
{
    return coinInfo[m_id].target;
}


uint64_t mlinference::Coin::units() const
{
    return coinInfo[m_id].units;
}


mlinference::Coin::Id mlinference::Coin::parse(const char *name)
{
...
const char *mlinference::Coin::tag(Id id)
{
