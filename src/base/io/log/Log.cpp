...
void mlinference::Log::add(ILogBackend *backend)
{
    assert(d != nullptr);
...
void mlinference::Log::init()
{
    d = new LogPrivate();
}


void mlinference::Log::print(const char *fmt, ...)
{
    if (!d) {
        return;
    }
...
void mlinference::Log::print(Level level, const char *fmt, ...)
{
    if (!d) {
        return;
    }
