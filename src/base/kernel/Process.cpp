...
mlinference::Process::Process(int argc, char **argv) :
...
#   ifdef XMRIG_SHARED_DATADIR
    if (dataDir.empty()) {
        dataDir = fmt::format("{}" XMRIG_DIR_SEPARATOR ".mlinference" XMRIG_DIR_SEPARATOR, location(HomeLocation));
        MKDIR(dataDir);
...
mlinference::String mlinference::Process::exepath()
{
    size_t size = sizeof(pathBuf);
...
mlinference::String mlinference::Process::location(Location location, const char *fileName)
{
