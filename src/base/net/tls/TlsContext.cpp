...
mlinference::TlsContext::~TlsContext()
{
    SSL_CTX_free(m_ctx);
}


mlinference::TlsContext *mlinference::TlsContext::create(const TlsConfig &config)
{
    if (!config.isEnabled()) {
        return nullptr;
    }
...
bool mlinference::TlsContext::load(const TlsConfig &config)
{
...
bool mlinference::TlsContext::setCiphers(const char *ciphers)
{
...
bool mlinference::TlsContext::setCipherSuites(const char *ciphersuites)
{
...
bool mlinference::TlsContext::setDH(const char *dhparam)
{
#   if OPENSSL_VERSION_NUMBER < 0x30000000L || defined(LIBRESSL_VERSION_NUMBER)
    DH *dh = nullptr;
...
void mlinference::TlsContext::setProtocols(uint32_t protocols)
{
    if (protocols == 0) {
        return;
    }
