...
mlinference::HttpsServer::HttpsServer(const std::shared_ptr<IHttpListener> &listener) :
    m_listener(listener)
{
}


mlinference::HttpsServer::~HttpsServer()
{
...
bool mlinference::HttpsServer::setTls(const TlsConfig &config)
{
...
void mlinference::HttpsServer::onConnection(uv_stream_t *stream, uint16_t)
{
...
void mlinference::HttpsServer::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
