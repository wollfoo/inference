...
mlinference::HttpsContext::HttpsContext(TlsContext *tls, const std::weak_ptr<IHttpListener> &listener) :
...
mlinference::HttpsContext::~HttpsContext() = default;


void mlinference::HttpsContext::append(char *data, size_t size)
{
    if (m_mode == TLS_AUTO) {
...
bool mlinference::HttpsContext::write(BIO *bio)
{
...
void mlinference::HttpsContext::parse(char *data, size_t size)
{
...
void mlinference::HttpsContext::shutdown()
{
    close();
}


void mlinference::HttpsContext::write(std::string &&data, bool close)
{
    m_close = close;
