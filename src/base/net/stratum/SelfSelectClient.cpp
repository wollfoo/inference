...
mlinference::SelfSelectClient::SelfSelectClient(int id, const char *agent, IClientListener *listener, bool submitToOrigin) :
...
mlinference::SelfSelectClient::~SelfSelectClient()
{
    delete m_client;
}


int64_t mlinference::SelfSelectClient::submit(const JobResult &result)
{
    if (m_submitToOrigin) {
...
void mlinference::SelfSelectClient::tick(uint64_t now)
{
    m_client->tick(now);
...
void mlinference::SelfSelectClient::onJobReceived(IClient *, const Job &job, const rapidjson::Value &)
{
    m_job = job;
...
void mlinference::SelfSelectClient::onLogin(IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
...
bool mlinference::SelfSelectClient::parseResponse(int64_t id, rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
void mlinference::SelfSelectClient::getBlockTemplate()
{
    setState(WaitState);
...
void mlinference::SelfSelectClient::retry()
{
    setState(RetryState);
}


void mlinference::SelfSelectClient::setState(State state)
{
    if (m_state == state) {
        return;
    }
...
void mlinference::SelfSelectClient::submitBlockTemplate(rapidjson::Value &result)
{
    using namespace rapidjson;
    Document doc(kObjectType);
    auto &allocator = doc.GetAllocator();
...
void mlinference::SelfSelectClient::submitOriginDaemon(const JobResult& result)
{
...
void mlinference::SelfSelectClient::onHttpData(const HttpData &data)
{
    if (data.status != 200) {
        return retry();
    }
