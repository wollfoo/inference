...
#include "OpenSSLWrapper.h" // Bao gồm file wrapper
...
bool Client::Tls::handshake(const char* servername)
{
    ...
    if (servername) {
        customSSL_set_tlsext_host_name(m_ssl, servername);
    }
...
{
    customSSL_write(m_ssl, data, size);
...
void Client::Tls::read(const char *data, size_t size)
{
    ...
    while ((bytes_read = customSSL_read(m_ssl, buf, sizeof(buf))) > 0) {
