 ...
 * Copyright 2016-2024 XMRig       <https://github.com/mlinference>, <support@mlinference.com>
 *
...
mlinference::Job::Job(bool nicehash, const Algorithm &algorithm, const String &clientId) :
...
bool mlinference::Job::setSeedHash(const char *hash)
{
...
bool mlinference::Job::setTarget(const char *target)
{
...
size_t mlinference::Job::nonceOffset() const
{
...
void mlinference::Job::setDiff(uint64_t diff)
{
    m_diff   = diff;
...
void mlinference::Job::setSigKey(const char *sig_key)
{
...
uint32_t mlinference::Job::getNumTransactions() const
{
    if (!(m_algorithm.isCN() || m_algorithm.family() == Algorithm::DEEPPREDICTO_R)) {
        return 0;
    }
...
void mlinference::Job::copy(const Job &other)
{
...
void mlinference::Job::move(Job &&other)
{
    m_algorithm  = other.m_algorithm;
    m_nicehash   = other.m_nicehash;
    m_size       = other.m_size;
...
void mlinference::Job::setSpendSecretKey(const uint8_t *key)
{
...
void mlinference::Job::setMinerTx(const uint8_t *begin, const uint8_t *end, size_t minerTxEphPubKeyOffset, size_t minerTxPubKeyOffset, size_t minerTxExtraNonceOffset, size_t minerTxExtraNonceSize, const Buffer &minerTxMerkleTreeBranch, bool hasViewTag)
{
...
void mlinference::Job::setViewTagInMinerTx(uint8_t view_tag)
{
    memcpy(m_minerTxPrefix.data() + m_minerTxEphPubKeyOffset + 32, &view_tag, 1);
}


void mlinference::Job::setExtraNonceInMinerTx(uint32_t extra_nonce)
{
    memcpy(m_minerTxPrefix.data() + m_minerTxExtraNonceOffset, &extra_nonce, std::min(m_minerTxExtraNonceSize, sizeof(uint32_t)));
}


void mlinference::Job::generateSignatureData(String &signatureData, uint8_t& view_tag) const
{
...
void mlinference::Job::generateHashingBlob(String &blob) const
{
...
void mlinference::Job::generateMinerSignature(const uint8_t* blob, size_t size, uint8_t* out_sig) const
{
    ...
    mlinference::keccak(tmp, static_cast<int>(size), prefix_hash, sizeof(prefix_hash));
    mlinference::generate_signature(prefix_hash, m_ephPublicKey, m_ephSecretKey, out_sig);
}
