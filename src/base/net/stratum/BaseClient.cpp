...
mlinference::BaseClient::BaseClient(int id, IClientListener *listener) :
    m_listener(listener),
    m_id(id)
{
}
...
void mlinference::BaseClient::setPool(const Pool &pool)
{
    if (!pool.isValid()) {
        return;
    }
...
bool mlinference::BaseClient::handleResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
    if (id == 1) {
        return false;
    }
...
bool mlinference::BaseClient::handleSubmitResponse(int64_t id, const char *error)
{
