 ...
 * Copyright (c) 2016-2024 XMRig       <https://github.com/mlinference>, <support@mlinference.com>
 *
...
mlinference::Client::Client(int id, const char *agent, IClientListener *listener) :
...
mlinference::Client::~Client()
{
    delete m_socket;
}


bool mlinference::Client::disconnect()
{
    m_keepAlive = 0;
    m_expire    = 0;
    m_failures  = -1;
...
bool mlinference::Client::isTLS() const
{
#   ifdef XMRIG_FEATURE_TLS
...
const char *mlinference::Client::tlsFingerprint() const
{
#   ifdef XMRIG_FEATURE_TLS
...
const char *mlinference::Client::tlsVersion() const
{
#   ifdef XMRIG_FEATURE_TLS
    if (isTLS()) {
...
int64_t mlinference::Client::send(const rapidjson::Value &obj, Callback callback)
{
...
int64_t mlinference::Client::send(const rapidjson::Value &obj)
{
...
int64_t mlinference::Client::submit(const JobResult &result)
{
...
void mlinference::Client::connect()
{
...
void mlinference::Client::connect(const Pool &pool)
{
    setPool(pool);
    connect();
}
...
void mlinference::Client::deleteLater()
{
    if (!m_listener) {
        return;
    }
...
void mlinference::Client::tick(uint64_t now)
{
...
void mlinference::Client::onResolved(const DnsRecords &records, int status, const char *error)
{
    m_dns.reset();
...
bool mlinference::Client::parseJob(const rapidjson::Value &params, int *code)
{
    if (!params.IsObject()) {
        *code = 2;
        return false;
...
#   endif
    ...
    if (m_pool.mode() != Pool::MODE_SELF_SELECT && job.algorithm().family() == Algorithm::DEEPPREDICTO_R && !job.setSeedHash(Json::getString(params, "seed_hash"))) {
        *code = 7;
        return false;
    }
    ...
    if (m_jobs == 0) { // https://github.com/mlinference/mlinference/issues/459
        return false;
    }
...
bool mlinference::Client::send(BIO *bio)
{
#   ifdef XMRIG_FEATURE_TLS
    uv_buf_t buf;
...
bool mlinference::Client::verifyAlgorithm(const Algorithm &algorithm, const char *algo) const
{
...
bool mlinference::Client::write(const uv_buf_t &buf)
{
...
int mlinference::Client::resolve(const String &host)
{
...
int64_t mlinference::Client::send(size_t size)
{
...
void mlinference::Client::connect(const sockaddr *addr)
{
...
void mlinference::Client::handshake()
{
    if (m_socks5) {
...
bool mlinference::Client::parseLogin(const rapidjson::Value &result, int *code)
{
...
void mlinference::Client::onClose()
{
    delete m_socket;
...
void mlinference::Client::parse(char *line, size_t len)
{
    startTimeout();
...
void mlinference::Client::parseExtensions(const rapidjson::Value &result)
{
    m_extensions.reset();
...
void mlinference::Client::parseNotification(const char *method, const rapidjson::Value &params, const rapidjson::Value &)
{
...
void mlinference::Client::parseResponse(int64_t id, const rapidjson::Value &result, const rapidjson::Value &error)
{
...
void mlinference::Client::read(ssize_t nread, const uv_buf_t *buf)
{
...
void mlinference::Client::reconnect()
{
    if (!m_listener) {
        m_storage.remove(m_key);
...
void mlinference::Client::setState(SocketState state)
{
...
void mlinference::Client::startTimeout()
{
    m_expire = 0;
...
bool mlinference::Client::isCriticalError(const char *message)
{
    if (!message) {
        return false;
    }
...
void mlinference::Client::onClose(uv_handle_t *handle)
{
...
void mlinference::Client::onConnect(uv_connect_t *req, int status)
{
...
void mlinference::Client::onRead(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
