 ...
 * Copyright 2016-2021 XMRig       <https://github.com/mlinference>, <support@mlinference.com>
 *
...
bool mlinference::Pools::isEqual(const Pools &other) const
{
...
int mlinference::Pools::donateLevel() const
{
#   ifdef XMRIG_FEATURE_BENCHMARK
...
mlinference::IStrategy *mlinference::Pools::createStrategy(IStrategyListener *listener) const
{
    if (active() == 1) {
...
rapidjson::Value mlinference::Pools::toJSON(rapidjson::Document &doc) const
{
...
size_t mlinference::Pools::active() const
{
    size_t count = 0;
    for (const Pool &pool : m_data) {
        if (pool.isEnabled()) {
...
void mlinference::Pools::load(const IJsonReader &reader)
{
    m_data.clear();
...
uint32_t mlinference::Pools::benchSize() const
{
#   ifdef XMRIG_FEATURE_BENCHMARK
...
void mlinference::Pools::print() const
{
    size_t i = 1;
    for (const Pool &pool : m_data) {
...
void mlinference::Pools::toJSON(rapidjson::Value &out, rapidjson::Document &doc) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void mlinference::Pools::setDonateLevel(int level)
{
...
void mlinference::Pools::setProxyDonate(int value)
{
    switch (value) {
...
void mlinference::Pools::setRetries(int retries)
{
...
void mlinference::Pools::setRetryPause(int retryPause)
{
