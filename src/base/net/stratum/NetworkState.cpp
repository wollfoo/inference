...
mlinference::NetworkState::NetworkState(IStrategyListener *listener) : StrategyProxy(listener)
{
}


#ifdef XMRIG_FEATURE_API
rapidjson::Value mlinference::NetworkState::getConnection(rapidjson::Document &doc, int version) const
{
...
rapidjson::Value mlinference::NetworkState::getResults(rapidjson::Document &doc, int version) const
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void mlinference::NetworkState::printConnection() const
{
    if (!m_active) {
...
void mlinference::NetworkState::printResults() const
{
    if (!m_hashes) {
...
const char *mlinference::NetworkState::scaleDiff(uint64_t &diff)
{
...
std::string mlinference::NetworkState::humanDiff(uint64_t diff)
{
...
void mlinference::NetworkState::onActive(IStrategy *strategy, IClient *client)
{
...
void mlinference::NetworkState::onJob(IStrategy *strategy, IClient *client, const Job &job, const rapidjson::Value &params)
{
...
void mlinference::NetworkState::onPause(IStrategy *strategy)
{
...
void mlinference::NetworkState::onResultAccepted(IStrategy *strategy, IClient *client, const SubmitResult &result, const char *error)
{
    add(result, error);
...
uint32_t mlinference::NetworkState::latency() const
{
...
uint64_t mlinference::NetworkState::avgTime() const
{
    if (m_latency.empty()) {
        return 0;
    }
...
uint64_t mlinference::NetworkState::connectionTime() const
{
    return m_active ? ((Chrono::steadyMSecs() - m_connectionTime)) : 0;
}


void mlinference::NetworkState::add(const SubmitResult &result, const char *error)
{
    if (error) {
        m_rejected++;
        return;
...
void mlinference::NetworkState::stop()
{
    m_active      = false;
    m_diff        = 0;
    m_ip          = nullptr;
