...
mlinference::BenchClient::BenchClient(const std::shared_ptr<BenchConfig> &benchmark, IClientListener* listener) :
    m_listener(listener),
...
#   ifdef XMRIG_FEATURE_HTTP
    if (m_benchmark->isSubmit() && (m_benchmark->algorithm().family() == Algorithm::DEEPPREDICTO_R)) {
        m_mode  = ONLINE_BENCH;
...
mlinference::BenchClient::~BenchClient()
{
    BenchState::destroy();
}


const char *mlinference::BenchClient::tag() const
{
    return Tags::bench();
}


void mlinference::BenchClient::connect()
{
#   ifdef XMRIG_FEATURE_HTTP
...
void mlinference::BenchClient::setPool(const Pool &pool)
{
    m_pool = pool;
}


void mlinference::BenchClient::onBenchDone(uint64_t result, uint64_t diff, uint64_t ts)
{
    m_result    = result;
    m_diff      = diff;
    m_doneTime  = ts;
...
void mlinference::BenchClient::onBenchReady(uint64_t ts, uint32_t threads, const IBackend *backend)
{
    m_readyTime = ts;
    m_threads   = threads;
    m_backend   = backend;
...
void mlinference::BenchClient::onHttpData(const HttpData &data)
{
#   ifdef XMRIG_FEATURE_HTTP
...
void mlinference::BenchClient::onResolved(const DnsRecords &records, int status, const char *error)
{
#   ifdef XMRIG_FEATURE_HTTP
...
bool mlinference::BenchClient::setSeed(const char *seed)
{
    if (!seed) {
        return false;
    }
...
uint64_t mlinference::BenchClient::referenceHash() const
{
...
void mlinference::BenchClient::printExit() const
{
    LOG_INFO("%s " WHITE_BOLD("press ") MAGENTA_BOLD("Ctrl+C") WHITE_BOLD(" to exit"), tag());
}


void mlinference::BenchClient::start()
{
...
#ifdef XMRIG_FEATURE_HTTP
void mlinference::BenchClient::onCreateReply(const rapidjson::Value &value)
{
...
void mlinference::BenchClient::onDoneReply(const rapidjson::Value &)
{
    LOG_NOTICE("%s " WHITE_BOLD("benchmark submitted ") CYAN_BOLD("https://mlinference.com/benchmark/%s"), tag(), m_job.id().data());
    printExit();
}


void mlinference::BenchClient::onGetReply(const rapidjson::Value &value)
{
...
void mlinference::BenchClient::resolve()
{
    m_dns = Dns::resolve(BenchConfig::kApiHost, this);
}


void mlinference::BenchClient::send(Request request)
{
...
void mlinference::BenchClient::setError(const char *message, const char *label)
{
...
void mlinference::BenchClient::update(const rapidjson::Value &body)
{
