...
const char *BenchConfig::kApiHost   = "api.mlinference.com";
#else
...
mlinference::BenchConfig::BenchConfig(uint32_t size, const String &id, const rapidjson::Value &object, bool dmi, uint32_t rotation) :
    ...
    m_rotation(rotation)
{
    ...
    if (!m_algorithm.isValid() || (f != Algorithm::DEEPPREDICTO_R
...
mlinference::BenchConfig *mlinference::BenchConfig::create(const rapidjson::Value &object, bool dmi)
{
...
rapidjson::Value mlinference::BenchConfig::toJSON(rapidjson::Document &doc) const
{
...
uint32_t mlinference::BenchConfig::getSize(const char *benchmark)
{
    if (!benchmark) {
        return 0;
    }
