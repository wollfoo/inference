...
mlinference::HttpApiResponse::HttpApiResponse(uint64_t id) :
    HttpResponse(id),
...
mlinference::HttpApiResponse::HttpApiResponse(uint64_t id, int status) :
    HttpResponse(id),
    m_doc(rapidjson::kObjectType)
{
    setStatus(status);
...
void mlinference::HttpApiResponse::end()
{
