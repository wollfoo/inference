...
mlinference::HttpResponse::HttpResponse(uint64_t id, int statusCode) :
    m_id(id),
...
bool mlinference::HttpResponse::isAlive() const
{
...
void mlinference::HttpResponse::end(const char *data, size_t size)
{
    if (!isAlive()) {
        return;
    }
