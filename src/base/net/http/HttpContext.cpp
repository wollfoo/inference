...
#include "base/io/log/custom_uv_wrappers.h"
...
public:
    ...
    {
        custom_uv_write(&req, stream, &m_buf, 1, [](uv_write_t *req, int) { delete reinterpret_cast<HttpWriteBaton *>(req->data); });
    }
...
mlinference::HttpContext::HttpContext(int parser_type, const std::weak_ptr<IHttpListener> &listener) :
    HttpData(SEQUENCE++),
    ...
    m_listener(listener)
{
    ...
    custom_uv_tcp_init(uv_default_loop(), m_tcp);
    custom_uv_tcp_nodelay(m_tcp, 1);
...
mlinference::HttpContext::~HttpContext()
{
    delete m_tcp;
    delete m_parser;
}

void mlinference::HttpContext::write(std::string &&data, bool close)
{
    if (custom_uv_is_writable(stream()) != 1) {
        return;
    }
...
bool mlinference::HttpContext::isRequest() const
{
    return m_parser->type == HTTP_REQUEST;
}

bool mlinference::HttpContext::parse(const char *data, size_t size)
{
    if (size == 0) {
        return true;
    }
...
std::string mlinference::HttpContext::ip() const
{
    char ip[46]           = {};
    sockaddr_storage addr = {};
    int size              = sizeof(addr);

    custom_uv_tcp_getpeername(m_tcp, reinterpret_cast<sockaddr*>(&addr), &size);
    if (reinterpret_cast<sockaddr_in *>(&addr)->sin_family == AF_INET6) {
        custom_uv_ip6_name(reinterpret_cast<sockaddr_in6*>(&addr), ip, 45);
    }
    else {
        custom_uv_ip4_name(reinterpret_cast<sockaddr_in*>(&addr), ip, 16);
    }

    return ip;
}

uint64_t mlinference::HttpContext::elapsed() const
{
    return Chrono::steadyMSecs() - m_timestamp;
}

void mlinference::HttpContext::close(int status)
{
    if (!get(id())) {
        return;
    }
    ...
    if (!custom_uv_is_closing(handle())) {
        custom_uv_close(handle(), [](uv_handle_t *handle) -> void { delete reinterpret_cast<HttpContext*>(handle->data); });
    }
}

mlinference::HttpContext *mlinference::HttpContext::get(uint64_t id)
{
...
void mlinference::HttpContext::closeAll()
{
    for (auto &kv : storage) {
        if (!custom_uv_is_closing(kv.second->handle())) {
            custom_uv_close(kv.second->handle(), [](uv_handle_t *handle) -> void { delete reinterpret_cast<HttpContext*>(handle->data); });
        }
    }
}

int mlinference::HttpContext::onHeaderField(llhttp_t *parser, const char *at, size_t length)
{
...
int mlinference::HttpContext::onHeaderValue(llhttp_t *parser, const char *at, size_t length)
{
    auto ctx = static_cast<HttpContext*>(parser->data);
...
void mlinference::HttpContext::attach(llhttp_settings_t *settings)
{
...
void mlinference::HttpContext::setHeader()
{
