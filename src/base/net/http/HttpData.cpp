...
#include "base/io/log/custom_uv_wrappers.h" // Include custom UV wrapper
...
bool mlinference::HttpData::isJSON() const
{
...
const char *mlinference::HttpData::methodName() const
{
    return llhttp_method_name(static_cast<llhttp_method>(method));
}


rapidjson::Document mlinference::HttpData::json() const
{
    if (status < 0) {
...
const char *mlinference::HttpData::statusName(int status)
{
    if (status < 0) {
        return custom_uv_strerror(status);  // Use custom_uv_strerror instead of uv_strerror
    }
