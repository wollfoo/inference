...
mlinference::Http::Http() :
    m_host(kLocalhost)
{
}


bool mlinference::Http::isEqual(const Http &other) const
{
...
rapidjson::Value mlinference::Http::toJSON(rapidjson::Document &doc) const
{
...
void mlinference::Http::load(const rapidjson::Value &http)
{
    if (!http.IsObject()) {
        return;
    }
...
void mlinference::Http::setPort(int port)
{
