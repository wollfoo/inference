...
#include "crypto/cn/CryptoNight_cortexai.h"
...
void v4_soft_aes_compile_code(const V4_Instruction *code, int code_size, void *machine_code, mlinference::Assembly ASM);
...
static inline void cryptonight_cortexai_tweak(uint64_t *mem_out, const uint8_t *l, uint64_t idx, __m128i ax0, __m128i bx0, __m128i bx1, __m128i& cx)
{
    constexpr CnAlgo<ALGO> props;
...
#   endif
    ...
    for (size_t i = 0; i < props.iterations(); i++) {
        ...
        if (BASE == Algorithm::CN_1 || BASE == Algorithm::CN_2) {
            cryptonight_cortexai_tweak<ALGO>(reinterpret_cast<uint64_t*>(&l0[interleaved_index<interleave>(idx0 & MASK)]), l0, idx0 & MASK, ax0, bx0, bx1, cx);
        } else {
...
void v4_compile_code(const V4_Instruction* code, int code_size, void* machine_code, mlinference::Assembly ASM);
void v4_compile_code_double(const V4_Instruction* code, int code_size, void* machine_code, mlinference::Assembly ASM);


template<mlinference::Algorithm::Id ALGO>
void cn_r_compile_code(const V4_Instruction* code, int code_size, void* machine_code, mlinference::Assembly ASM)
{
...
template<mlinference::Algorithm::Id ALGO>
void cn_r_compile_code_double(const V4_Instruction* code, int code_size, void* machine_code, mlinference::Assembly ASM)
{
...
#   endif
    ...
    for (size_t i = 0; i < props.iterations(); i++) {
        ...
        if (BASE == Algorithm::CN_1 || BASE == Algorithm::CN_2) {
            cryptonight_cortexai_tweak<ALGO>((uint64_t*)&l0[idx0 & MASK], l0, idx0 & MASK, ax0, bx00, bx01, cx0);
            cryptonight_cortexai_tweak<ALGO>((uint64_t*)&l1[idx1 & MASK], l1, idx1 & MASK, ax1, bx10, bx11, cx1);
        } else {
...
#define CN_STEP2(a, b0, b1, c, l, ptr, idx)                                             \
    if (IS_CN_HEAVY_TUBE) {                                                             \
        ...
        c = _mm_aesenc_si128(c, a);                                                     \
                                                                                        ...
                                                                                        \
    if (BASE == Algorithm::CN_1 || BASE == Algorithm::CN_2) {                           \
        cryptonight_cortexai_tweak<ALGO>((uint64_t*)ptr, l, idx & MASK, a, b0, b1, c);    \
    } else {                                                                            \
