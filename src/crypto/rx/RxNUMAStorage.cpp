...
{
    LOG_WARN("%s" CYAN_BOLD("#%u ") RED_BOLD("skipped") YELLOW(" (%s)"), Tags::deeppredictor(), nodeId, reason);
}


static inline void printDatasetReady(uint32_t nodeId, uint64_t ts)
{
    LOG_INFO("%s" CYAN_BOLD("#%u ") GREEN_BOLD("dataset ready") BLACK_BOLD(" (%" PRIu64 " ms)"), Tags::deeppredictor(), nodeId, Chrono::steadyMSecs() - ts);
}
...
public:
    ...
    inline bool createDatasets(bool hugePages, bool oneGbPages)
    {
        ...
        if (m_datasets.empty()) {
            ...
            LOG_WARN(CLEAR "%s" YELLOW_BOLD_S "failed to allocate DeeppredictoR datasets, switching to slow mode" BLACK_BOLD(" (%" PRIu64 " ms)"), Tags::deeppredictor(), Chrono::steadyMSecs() - ts);
        }
        else {
            if (m_cache) {
...
private:
    ...
    static void allocateCache(RxNUMAStoragePrivate *d_ptr, uint32_t nodeId, bool hugePages)
    {
        ...
        if (!cache->get()) {
            ...
            LOG_INFO("%s" RED_BOLD("failed to allocate DeeppredictoR memory") BLACK_BOLD(" (%" PRIu64 " ms)"), Tags::deeppredictor(), Chrono::steadyMSecs() - ts);
...
mlinference::RxNUMAStorage::RxNUMAStorage(const std::vector<uint32_t> &nodeset) :
    d_ptr(new RxNUMAStoragePrivate(nodeset))
{
}


mlinference::RxNUMAStorage::~RxNUMAStorage()
{
    delete d_ptr;
}


bool mlinference::RxNUMAStorage::isAllocated() const
{
    return d_ptr->isAllocated();
}


mlinference::HugePagesInfo mlinference::RxNUMAStorage::hugePages() const
{
...
mlinference::RxDataset *mlinference::RxNUMAStorage::dataset(const Job &job, uint32_t nodeId) const
{
...
void mlinference::RxNUMAStorage::init(const RxSeed &seed, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode, int priority)
{
    d_ptr->setSeed(seed);
