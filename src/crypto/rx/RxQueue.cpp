...
mlinference::RxQueue::RxQueue(IRxListener *listener) :
    m_listener(listener)
{
...
mlinference::RxDataset *mlinference::RxQueue::dataset(const Job &job, uint32_t nodeId)
{
...
mlinference::HugePagesInfo mlinference::RxQueue::hugePages()
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
template<typename T>
bool mlinference::RxQueue::isReady(const T &seed)
{
    std::lock_guard<std::mutex> lock(m_mutex);
...
void mlinference::RxQueue::enqueue(const RxSeed &seed, const std::vector<uint32_t> &nodeset, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode mode, int priority)
{
    std::unique_lock<std::mutex> lock(m_mutex);
...
template<typename T>
bool mlinference::RxQueue::isReadyUnsafe(const T &seed) const
{
    return m_storage != nullptr && m_storage->isAllocated() && m_state == STATE_IDLE && m_seed == seed;
}


void mlinference::RxQueue::backgroundInit()
{
...
void mlinference::RxQueue::onReady()
{
    std::unique_lock<std::mutex> lock(m_mutex);
