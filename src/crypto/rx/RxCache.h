...
public:
    ...
    inline deeppredictor_cache *get() const       { return m_cache; }
    inline size_t size() const              { return maxSize(); }

    bool init(const Buffer &seed);
    HugePagesInfo hugePages() const;

    static inline constexpr size_t maxSize() { return DEEPPREDICTOR_CACHE_MAX_SIZE; }
...
private:
    ...
    bool m_jit              = true;
    Buffer m_seed;
    deeppredictor_cache *m_cache  = nullptr;
