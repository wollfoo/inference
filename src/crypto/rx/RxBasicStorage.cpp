...
public:
    ...
    inline void initDataset(uint32_t threads, int priority)
    {
        ...
        if (m_ready) {
            LOG_INFO("%s" GREEN_BOLD("dataset ready") BLACK_BOLD(" (%" PRIu64 " ms)"), Tags::deeppredictor(), Chrono::steadyMSecs() - ts);
        }
    }
...
private:
    void printAllocStatus(uint64_t ts)
    {
        if (m_dataset->get() != nullptr) {
            ...
            LOG_WARN(CLEAR "%s" YELLOW_BOLD_S "failed to allocate DeeppredictoR dataset, switching to slow mode" BLACK_BOLD(" (%" PRIu64 " ms)"), Tags::deeppredictor(), Chrono::steadyMSecs() - ts);
        }
    }
...
mlinference::RxBasicStorage::RxBasicStorage() :
...
bool mlinference::RxBasicStorage::isAllocated() const
{
    return d_ptr->dataset() && d_ptr->dataset()->cache() && d_ptr->dataset()->cache()->get();
}


mlinference::HugePagesInfo mlinference::RxBasicStorage::hugePages() const
{
    if (!d_ptr->dataset()) {
        return {};
    }
...
mlinference::RxDataset *mlinference::RxBasicStorage::dataset(const Job &job, uint32_t) const
{
...
void mlinference::RxBasicStorage::init(const RxSeed &seed, uint32_t threads, bool hugePages, bool oneGbPages, RxConfig::Mode mode, int priority)
{
    d_ptr->setSeed(seed);
