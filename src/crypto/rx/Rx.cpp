...
mlinference::HugePagesInfo mlinference::Rx::hugePages()
{
    return d_ptr->queue.hugePages();
}


mlinference::RxDataset *mlinference::Rx::dataset(const Job &job, uint32_t nodeId)
{
    return d_ptr->queue.dataset(job, nodeId);
}


void mlinference::Rx::destroy()
{
#   ifdef XMRIG_FEATURE_MSR
    RxMsr::destroy();
#   endif
...
void mlinference::Rx::init(IRxListener *listener)
{
...
#include "crypto/deeppredictor/blake2/avx2/blake2b.h"
#endif
...
template<typename T>
bool mlinference::Rx::init(const T &seed, const RxConfig &config, const CpuConfig &cpu)
{
    const auto f = seed.algorithm().family();
    if ((f != Algorithm::DEEPPREDICTO_R)
...
#   endif
    ...
    deeppredictor_set_scratchpad_prefetch_mode(config.scratchpadPrefetchMode());
    deeppredictor_set_huge_pages_jit(cpu.isHugePagesJit());
    deeppredictor_set_optimized_dataset_init(config.initDatasetAVX2());
...
template<typename T>
bool mlinference::Rx::isReady(const T &seed)
{
