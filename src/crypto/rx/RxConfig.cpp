...
const char *RxConfig::kField                    = "deeppredictor";
...
bool mlinference::RxConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
rapidjson::Value mlinference::RxConfig::toJSON(rapidjson::Document &doc) const
{
...
#ifdef XMRIG_FEATURE_HWLOC
std::vector<uint32_t> mlinference::RxConfig::nodeset() const
{
    if (!m_nodeset.empty()) {
        return m_nodeset;
    }
...
const char *mlinference::RxConfig::modeName() const
{
    return modeNames[m_mode];
}


uint32_t mlinference::RxConfig::threads(uint32_t limit) const
{
    if (m_threads > 0) {
        return m_threads;
    }
...
#ifdef XMRIG_FEATURE_MSR
const char *mlinference::RxConfig::msrPresetName() const
{
    return modNames[msrMod()];
}


const mlinference::MsrItems &mlinference::RxConfig::msrPreset() const
{
...
uint32_t mlinference::RxConfig::msrMod() const
{
    if (!wrmsr()) {
...
void mlinference::RxConfig::readMSR(const rapidjson::Value &value)
{
    if (value.IsBool()) {
...
mlinference::RxConfig::Mode mlinference::RxConfig::readMode(const rapidjson::Value &value)
{
    if (value.IsUint()) {
