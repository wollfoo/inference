...
mlinference::MemoryPool::MemoryPool(size_t size, bool hugePages, uint32_t node)
{
    if (!size) {
        return;
    }
...
mlinference::MemoryPool::~MemoryPool()
{
    delete m_memory;
}


bool mlinference::MemoryPool::isHugePages(uint32_t) const
{
    return m_memory && m_memory->isHugePages();
}


uint8_t *mlinference::MemoryPool::get(size_t size, uint32_t)
{
...
void mlinference::MemoryPool::release(uint32_t)
{
    assert(m_refs > 0);
