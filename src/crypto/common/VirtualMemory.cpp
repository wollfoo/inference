...
mlinference::VirtualMemory::VirtualMemory(size_t size, bool hugePages, bool oneGbPages, bool usePool, uint32_t node, size_t alignSize) :
...
mlinference::VirtualMemory::~VirtualMemory()
{
    if (!m_scratchpad) {
        return;
    }
...
mlinference::HugePagesInfo mlinference::VirtualMemory::hugePages() const
{
    return { this };
}
...
uint32_t mlinference::VirtualMemory::bindToNUMANode(int64_t)
{
    return 0;
}
#endif
...
void mlinference::VirtualMemory::destroy()
{
    delete pool;
}


void mlinference::VirtualMemory::init(size_t poolSize, size_t hugePageSize)
{
    if (!pool) {
        osInit(hugePageSize);
    }
