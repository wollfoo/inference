...
static void getResults(JobBundle &bundle, std::vector<JobResult> &results, uint32_t &errors, bool hwAES)
{
    ...
    if (algorithm.family() == Algorithm::DEEPPREDICTO_R) {
#       ifdef XMRIG_ALGO_DEEPPREDICTOR
        ...
        for (uint32_t nonce : bundle.nonces) {
            *bundle.job.nonce() = nonce;
            ...
            deeppredictor_calculate_hash(vm, bundle.job.blob(), bundle.job.size(), hash);
...
void mlinference::JobResults::done(const Job &job)
{
    submit(JobResult(job));
}


void mlinference::JobResults::setListener(IJobResultListener *listener, bool hwAES)
{
...
void mlinference::JobResults::stop()
{
    assert(handler != nullptr);
...
void mlinference::JobResults::submit(const Job &job, uint32_t nonce, const uint8_t *result)
{
    submit(JobResult(job, nonce, result));
}


void mlinference::JobResults::submit(const Job& job, uint32_t nonce, const uint8_t* result, const uint8_t* miner_signature)
{
    submit(JobResult(job, nonce, result, nullptr, nullptr, miner_signature));
}


void mlinference::JobResults::submit(const JobResult &result)
{
    assert(handler != nullptr);
...
#if defined(XMRIG_FEATURE_OPENCL) || defined(XMRIG_FEATURE_CUDA)
void mlinference::JobResults::submit(const Job &job, uint32_t *results, size_t count, uint32_t device_index)
{
    if (handler) {
