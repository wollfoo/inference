...
// Bọc hàm uv_hrtime
static uint64_t custom_uv_hrtime() {
    return uv_hrtime();
}
...
static uint64_t benchmark_impl(const argon2_impl *impl) {
    ...
    /* OK, now measure: */
    const uint64_t time = custom_uv_hrtime();  // Sử dụng hàm custom bọc uv_hrtime

    for (uint32_t i = 0; i < BENCH_SAMPLES; i++) {
        impl->fill_segment(&instance, pos);
    }

    return custom_uv_hrtime() - time;  // Sử dụng hàm custom bọc uv_hrtime
}
...
void mlinference_ar2_fill_segment(const argon2_instance_t *instance, argon2_position_t position)
{
