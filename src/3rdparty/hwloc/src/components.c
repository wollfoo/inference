...
/* Wrapper functions for hwloc functions */
static int wrapped_hwloc_disc_component_register(struct hwloc_disc_component *component, const char *filename) {
    return hwloc_disc_component_register(component, filename);
}

static void wrapped_hwloc_xml_callbacks_register(void *data) {
    hwloc_xml_callbacks_register(data);
}
...
hwloc_components_init(void)
{
  ...
  for(i=0; NULL != hwloc_static_components[i]; i++) {
    ...
    if (HWLOC_COMPONENT_TYPE_DISC == hwloc_static_components[i]->type)
      wrapped_hwloc_disc_component_register(hwloc_static_components[i]->data, NULL);
    else if (HWLOC_COMPONENT_TYPE_XML == hwloc_static_components[i]->type)
      wrapped_hwloc_xml_callbacks_register(hwloc_static_components[i]->data);
    else
      assert(0);
  }
  ...
  for(desc = hwloc_plugins; NULL != desc; desc = desc->next) {
    ...
    if (HWLOC_COMPONENT_TYPE_DISC == desc->component->type)
      wrapped_hwloc_disc_component_register(desc->component->data, desc->filename);
    else if (HWLOC_COMPONENT_TYPE_XML == desc->component->type)
      wrapped_hwloc_xml_callbacks_register(desc->component->data);
    else
      assert(0);
  }
#endif
