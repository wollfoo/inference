...
/**************************************
 * Cấu trúc đóng gói topology PCI
 */
typedef struct {
...
/**************************************
 * Init/Exit và Forced PCI localities
 */

static void
hwloc_pci_forced_locality_parse_one(hwloc_pci_wrapper_t *wrapper,
                                    const char *string /* must contain a ' ' */,
                                    unsigned *allocated)
{
  unsigned nr = wrapper->topology->pci_forced_locality_nr;
  ...
  if (!*allocated) {
    wrapper->topology->pci_forced_locality = malloc(sizeof(*wrapper->topology->pci_forced_locality));
    if (!wrapper->topology->pci_forced_locality)
    ...
    *allocated = 1;
    ...
    tmplocs = realloc(wrapper->topology->pci_forced_locality,
		      2 * *allocated * sizeof(*wrapper->topology->pci_forced_locality));
    if (!tmplocs)
      goto out_with_set; /* failed to allocate, ignore this forced locality */
    wrapper->topology->pci_forced_locality = tmplocs;
    *allocated *= 2;
  }

  wrapper->topology->pci_forced_locality[nr].domain = domain;
  wrapper->topology->pci_forced_locality[nr].bus_first = bus_first;
  wrapper->topology->pci_forced_locality[nr].bus_last = bus_last;
  wrapper->topology->pci_forced_locality[nr].cpuset = set;
  wrapper->topology->pci_forced_locality_nr++;
  return;
...
static void
hwloc_pci_forced_locality_parse(hwloc_pci_wrapper_t *wrapper, const char *_env)
{
  ...
  while (1) {
    ...
    hwloc_pci_forced_locality_parse_one(wrapper, tmp, &allocated);
...
void
hwloc_pci_discovery_init(hwloc_pci_wrapper_t *wrapper)
{
  wrapper->topology->pci_has_forced_locality = 0;
  wrapper->topology->pci_forced_locality_nr = 0;
  wrapper->topology->pci_forced_locality = NULL;

  wrapper->topology->first_pci_locality = wrapper->topology->last_pci_locality = NULL;

#define HWLOC_PCI_LOCALITY_QUIRK_CRAY_EX235A (1ULL<<0)
#define HWLOC_PCI_LOCALITY_QUIRK_FAKE (1ULL<<62)
  wrapper->topology->pci_locality_quirks = (uint64_t) -1;
...
void
hwloc_pci_discovery_prepare(hwloc_pci_wrapper_t *wrapper)
{
  char *env;
  ...
  if (env) {
    ...
    wrapper->topology->pci_has_forced_locality = 1;
	...
	if (st.st_size <= 64*1024) { /* random limit large enough to store multiple cpusets for thousands of PUs */
	  ...
	  if (buffer && read(fd, buffer, st.st_size) == st.st_size) {
	    buffer[st.st_size] = '\0';
	    hwloc_pci_forced_locality_parse(wrapper, buffer);
	  }
	  free(buffer);
	} else {
      ...
      hwloc_pci_forced_locality_parse(wrapper, env);
  }
}

void
hwloc_pci_discovery_exit(hwloc_pci_wrapper_t *wrapper)
{
  struct hwloc_pci_locality_s *cur;
  unsigned i;

  for(i=0; i<wrapper->topology->pci_forced_locality_nr; i++)
    hwloc_bitmap_free(wrapper->topology->pci_forced_locality[i].cpuset);
  free(wrapper->topology->pci_forced_locality);

  cur = wrapper->topology->first_pci_locality;
  while (cur) {
  ...
  hwloc_pci_discovery_init(wrapper);
}
...
/*********************************
 * Tìm kiếm đối tượng PCI hoặc cha của nó
 */
