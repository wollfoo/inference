...
int main(int argc, char **argv) {
    ...
    if (err) {
        fprintf(stderr, "Lỗi khởi tạo hwloc topology\n");
        return -1;
    }

    // Nạp topology từ hệ thống
    err = hwloc_topology_load(topology);
    if (err) {
        fprintf(stderr, "Lỗi nạp hwloc topology\n");
        hwloc_topology_destroy(topology);
        return -1;
    }

    // Phần mã gốc của bạn bắt đầu từ đây
        ...
        if (*attr >= '0' && *attr <= '9') {
          ...
          while (tmp) {
            ...
            if (tmp3 == tmp2 || (*tmp3 && *tmp3 != ':' && *tmp3 != ')' && *tmp3 != ' ')) {
              if (verbose)
                fprintf(stderr, "Failed to read synthetic index interleaving loop '%s' without number giữa '*' và ':'\n", tmp);
              free(loops);
              goto out_with_array;
            }
            if (!nb) {
              if (verbose)
                fprintf(stderr, "Invalid interleaving loop with number 0 tại '%s'\n", tmp2);
              free(loops);
              goto out_with_array;
            }
				...
				unsigned long flags)
{
  ...
  return ret;
    ...
    // Giải phóng topology sau khi sử dụng
    hwloc_topology_destroy(topology);
    return 0;
}
