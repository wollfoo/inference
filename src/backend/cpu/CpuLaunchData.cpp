...
mlinference::CpuLaunchData::CpuLaunchData(const Miner *miner, const Algorithm &algorithm, const CpuConfig &config, const CpuThread &thread, size_t threads, const std::vector<int64_t>& affinities) :
    algorithm(algorithm),
...
bool mlinference::CpuLaunchData::isEqual(const CpuLaunchData &other) const
{
...
mlinference::CnHash::AlgoVariant mlinference::CpuLaunchData::av() const
{
    if (intensity <= 2) {
...
const char *mlinference::CpuLaunchData::tag()
{
    return cpu_tag();
}
