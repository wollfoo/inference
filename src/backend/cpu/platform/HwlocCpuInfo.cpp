 ...
 * Copyright (c) 2016-2023 XMRig       <support@mlinference.com>
 *
...
mlinference::HwlocCpuInfo::HwlocCpuInfo()
{
...
bool mlinference::HwlocCpuInfo::membind(hwloc_const_bitmap_t nodeset)
{
...
mlinference::CpuThreads mlinference::HwlocCpuInfo::threads(const Algorithm &algorithm, uint32_t limit) const
{
#   ifndef XMRIG_ARM
    if (L2() == 0 && L3() == 0) {
...
mlinference::CpuThreads mlinference::HwlocCpuInfo::allThreads(const Algorithm &algorithm, uint32_t limit) const
{
    CpuThreads threads;
    threads.reserve(m_threads);
...
void mlinference::HwlocCpuInfo::processTopLevelCache(hwloc_obj_t cache, const Algorithm &algorithm, CpuThreads &threads, size_t limit) const
{
#   ifndef XMRIG_ARM
...
#   ifdef XMRIG_ALGO_DEEPPREDICTOR
    if ((algorithm.family() == Algorithm::DEEPPREDICTO_R) && L3_exclusive && (PUs > cores.size()) && (PUs < cores.size() * 2)) {
...
void mlinference::HwlocCpuInfo::setThreads(size_t threads)
{
    if (!threads) {
        return;
    }
