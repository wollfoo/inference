...
template<size_t N>
mlinference::CpuWorker<N>::CpuWorker(size_t id, const CpuLaunchData &data) :
...
template<size_t N>
mlinference::CpuWorker<N>::~CpuWorker()
{
#   ifdef XMRIG_ALGO_DEEPPREDICTOR
    RxVm::destroy(m_vm);
#   endif
...
#ifdef XMRIG_ALGO_DEEPPREDICTOR
template<size_t N>
void mlinference::CpuWorker<N>::allocateDeeppredictoR_VM()
{
    ...
    if (!m_vm) {
        ...
        // Update DeeppredictoR light VM with the new seed
        deeppredictor_vm_set_cache(m_vm, dataset->cache()->get());
    }
...
template<size_t N>
bool mlinference::CpuWorker<N>::selfTest()
{
#   ifdef XMRIG_ALGO_DEEPPREDICTOR
    if (m_algorithm.family() == Algorithm::DEEPPREDICTO_R) {
        return N == 1;
    }
#   endif
...
template<size_t N>
void mlinference::CpuWorker<N>::hashrateData(uint64_t &hashCount, uint64_t &, uint64_t &rawHashes) const
{
    hashCount = m_count;
    rawHashes = m_count;
}
...
template<size_t N>
void mlinference::CpuWorker<N>::start()
{
...
#           ifdef XMRIG_ALGO_DEEPPREDICTOR
            ...
            if (job.algorithm().family() == Algorithm::DEEPPREDICTO_R) {
                if (first) {
                    first = false;
                    if (job.hasMinerSignature()) {
                        job.generateMinerSignature(m_job.blob(), job.size(), miner_signature_ptr);
                    }
                    deeppredictor_calculate_hash_first(m_vm, tempHash, m_job.blob(), job.size());
                }
                ...
                deeppredictor_calculate_hash_next(m_vm, tempHash, m_job.blob(), job.size(), m_hash);
            }
            else
#           endif
            {
...
template<size_t N>
bool mlinference::CpuWorker<N>::nextRound()
{
#   ifdef XMRIG_FEATURE_BENCHMARK
...
template<size_t N>
bool mlinference::CpuWorker<N>::verify(const Algorithm &algorithm, const uint8_t *referenceValue)
{
#   ifdef XMRIG_ALGO_GHOSTRIDER
...
template<size_t N>
bool mlinference::CpuWorker<N>::verify2(const Algorithm &algorithm, const uint8_t *referenceValue)
{
    cn_hash_fun func = fn(algorithm);
    if (!func) {
        return false;
...
template<size_t N>
void mlinference::CpuWorker<N>::allocateCnCtx()
{
    if (m_ctx[0] == nullptr) {
        int shift = 0;
...
template<size_t N>
void mlinference::CpuWorker<N>::consumeJob()
{
    if (Nonce::sequence(Nonce::CPU) == 0) {
        return;
    }
...
#   ifdef XMRIG_ALGO_DEEPPREDICTOR
    if (m_job.currentJob().algorithm().family() == Algorithm::DEEPPREDICTO_R) {
        allocateDeeppredictoR_VM();
    }
    else
#   endif
    {
