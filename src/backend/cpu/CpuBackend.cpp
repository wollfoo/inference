...
#       endif
    ...
    #   ifdef XMRIG_ALGO_DEEPPREDICTOR
        if (algo.family() == Algorithm::DEEPPREDICTO_R) {
            pages += Rx::hugePages();
        }
    #   endif
...
const char *mlinference::backend_tag(uint32_t backend)
{
...
const char *mlinference::cpu_tag()
{
    return Tags::cpu();
}


mlinference::CpuBackend::CpuBackend(Controller *controller) :
...
mlinference::CpuBackend::~CpuBackend()
{
    delete d_ptr;
}


bool mlinference::CpuBackend::isEnabled() const
{
    return d_ptr->controller->config()->cpu().isEnabled();
}


bool mlinference::CpuBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cpu().threads().get(algorithm).isEmpty();
}


bool mlinference::CpuBackend::tick(uint64_t ticks)
{
    return d_ptr->workers.tick(ticks);
}


const mlinference::Hashrate *mlinference::CpuBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const mlinference::String &mlinference::CpuBackend::profileName() const
{
    return d_ptr->profileName;
}


const mlinference::String &mlinference::CpuBackend::type() const
{
    return kType;
}


void mlinference::CpuBackend::prepare(const Job &nextJob)
{
#   ifdef XMRIG_ALGO_ARGON2
    const auto f = nextJob.algorithm().family();
    if ((f == Algorithm::ARGON2) || (f == Algorithm::DEEPPREDICTO_R)) {
...
void mlinference::CpuBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void mlinference::CpuBackend::printHealth()
{
}


void mlinference::CpuBackend::setJob(const Job &job)
{
    if (!isEnabled()) {
        return stop();
    }
...
void mlinference::CpuBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void mlinference::CpuBackend::stop()
{
...
#ifdef XMRIG_FEATURE_API
rapidjson::Value mlinference::CpuBackend::toJSON(rapidjson::Document &doc) const
{
...
void mlinference::CpuBackend::handleRequest(IApiRequest &request)
{
...
#ifdef XMRIG_FEATURE_BENCHMARK
mlinference::Benchmark *mlinference::CpuBackend::benchmark() const
{
    return d_ptr->benchmark.get();
}


void mlinference::CpuBackend::printBenchProgress() const
{
    if (d_ptr->benchmark) {
