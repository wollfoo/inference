...
mlinference::Benchmark::Benchmark(size_t workers, const IBackend *backend) :
    m_backend(backend),
    m_workers(workers)
{
}
...
bool mlinference::Benchmark::finish(uint64_t totalHashCount)
{
...
void mlinference::Benchmark::start()
{
    m_startTime = BenchState::start(m_workers, m_backend);
}


void mlinference::Benchmark::printProgress() const
{
