...
template<class T>
bool mlinference::Workers<T>::tick(uint64_t)
{
    if (!d_ptr->hashrate) {
        return true;
    }
...
template<class T>
const mlinference::Hashrate *mlinference::Workers<T>::hashrate() const
{
...
template<class T>
void mlinference::Workers<T>::setBackend(IBackend *backend)
{
...
template<class T>
void mlinference::Workers<T>::stop()
{
...
#ifdef XMRIG_FEATURE_BENCHMARK
template<class T>
void mlinference::Workers<T>::start(const std::vector<T> &data, const std::shared_ptr<Benchmark> &benchmark)
{
    if (!benchmark) {
...
template<class T>
mlinference::IWorker *mlinference::Workers<T>::create(Thread<T> *)
{
    return nullptr;
}
...
template<class T>
void *mlinference::Workers<T>::onReady(void *arg)
{
...
template<class T>
void mlinference::Workers<T>::start(const std::vector<T> &data, bool /*sleep*/)
{
...
template<>
mlinference::IWorker *mlinference::Workers<CpuLaunchData>::create(Thread<CpuLaunchData> *handle)
{
#   ifdef XMRIG_MINER_PROJECT
...
#ifdef XMRIG_FEATURE_OPENCL
template<>
mlinference::IWorker *mlinference::Workers<OclLaunchData>::create(Thread<OclLaunchData> *handle)
{
...
#ifdef XMRIG_FEATURE_CUDA
template<>
mlinference::IWorker *mlinference::Workers<CudaLaunchData>::create(Thread<CudaLaunchData> *handle)
{
