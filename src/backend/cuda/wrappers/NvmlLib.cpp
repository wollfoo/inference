...
bool mlinference::NvmlLib::init(const char *fileName)
{
    if (!m_initialized) {
        m_loader      = fileName;
        m_ready       = dlopen() && load();
...
const char *mlinference::NvmlLib::lastError() noexcept
{
    return uv_dlerror(&nvmlLib);
}


void mlinference::NvmlLib::close()
{
    if (m_ready) {
        pNvmlShutdown();
    }
...
bool mlinference::NvmlLib::assign(std::vector<CudaDevice> &devices)
{
    uint32_t count = 0;
...
NvmlHealth mlinference::NvmlLib::health(nvmlDevice_t device)
{
    if (!device) {
        return {};
    }
...
bool mlinference::NvmlLib::dlopen()
{
    if (!m_loader.isNull()) {
