...
#include "base/io/log/custom_uv_wrappers.h"  // Include custom UV wrappers
...
static String defaultLoader = "libmlinference-cuda.dylib";
#elif defined(_WIN32)
static String defaultLoader = "mlinference-cuda.dll";
#else
static String defaultLoader = "libmlinference-cuda.so";
#endif
...
#define DLSYM(x) if (custom_uv_dlsym(&cudaLib, k##x, reinterpret_cast<void**>(&p##x)) == -1) { throw std::runtime_error(std::string("symbol not found: ") + k##x); }
...
bool mlinference::CudaLib::init(const char *fileName)
{
    if (!m_initialized) {
        m_initialized = true;
...
const char *mlinference::CudaLib::lastError() noexcept
{
    return m_error;
}


void mlinference::CudaLib::close()
{
    custom_uv_dlclose(&cudaLib);
}


bool mlinference::CudaLib::cnHash(nvid_ctx *ctx, uint32_t startNonce, uint64_t height, uint64_t target, uint32_t *rescount, uint32_t *resnonce)
{
    return pCnHash(ctx, startNonce, height, target, rescount, resnonce);
}


bool mlinference::CudaLib::deviceInfo(nvid_ctx *ctx, int32_t blocks, int32_t threads, const Algorithm &algorithm, int32_t dataset_host) noexcept
{
...
bool mlinference::CudaLib::deviceInit(nvid_ctx *ctx) noexcept
{
    return pDeviceInit(ctx);
}


bool mlinference::CudaLib::rxHash(nvid_ctx *ctx, uint32_t startNonce, uint64_t target, uint32_t *rescount, uint32_t *resnonce) noexcept
{
    return pRxHash(ctx, startNonce, target, rescount, resnonce);
}


bool mlinference::CudaLib::rxPrepare(nvid_ctx *ctx, const void *dataset, size_t datasetSize, bool dataset_host, uint32_t batchSize) noexcept
{
    return pRxPrepare(ctx, dataset, datasetSize, dataset_host, batchSize);
}


bool mlinference::CudaLib::kawPowHash(nvid_ctx *ctx, uint8_t* job_blob, uint64_t target, uint32_t *rescount, uint32_t *resnonce, uint32_t *skipped_hashes) noexcept
{
    return pKawPowHash(ctx, job_blob, target, rescount, resnonce, skipped_hashes);
}


bool mlinference::CudaLib::kawPowPrepare(nvid_ctx *ctx, const void* cache, size_t cache_size, const void* dag_precalc, size_t dag_size, uint32_t height, const uint64_t* dag_sizes) noexcept
{
    return pKawPowPrepare_v2(ctx, cache, cache_size, dag_precalc, dag_size, height, dag_sizes);
}


bool mlinference::CudaLib::kawPowStopHash(nvid_ctx *ctx) noexcept
{
    return pKawPowStopHash(ctx);
}


bool mlinference::CudaLib::setJob(nvid_ctx *ctx, const void *data, size_t size, const Algorithm &algorithm) noexcept
{
    const Algorithm algo = RxAlgo::id(algorithm);
    if (pSetJob) {
...
const char *mlinference::CudaLib::deviceName(nvid_ctx *ctx) noexcept
{
    return pDeviceName(ctx);
}


const char *mlinference::CudaLib::lastError(nvid_ctx *ctx) noexcept
{
    return pLastError(ctx);
}


const char *mlinference::CudaLib::pluginVersion() noexcept
{
    return pPluginVersion();
}


int32_t mlinference::CudaLib::deviceInt(nvid_ctx *ctx, DeviceProperty property) noexcept
{
    return pDeviceInt(ctx, property);
}


nvid_ctx *mlinference::CudaLib::alloc(uint32_t id, int32_t bfactor, int32_t bsleep) noexcept
{
    return pAlloc(id, bfactor, bsleep);
}


std::string mlinference::CudaLib::version(uint32_t version)
{
    return std::to_string(version / 1000) + "." + std::to_string((version % 1000) / 10);
}


std::vector<mlinference::CudaDevice> mlinference::CudaLib::devices(int32_t bfactor, int32_t bsleep, const std::vector<uint32_t> &hints) noexcept
{
...
uint32_t mlinference::CudaLib::deviceCount() noexcept
{
    return pDeviceCount();
}


uint32_t mlinference::CudaLib::deviceUint(nvid_ctx *ctx, DeviceProperty property) noexcept
{
    return pDeviceUint(ctx, property);
}


uint32_t mlinference::CudaLib::driverVersion() noexcept
{
    return pVersion(DriverVersion);
}


uint32_t mlinference::CudaLib::runtimeVersion() noexcept
{
    return pVersion(RuntimeVersion);
}


uint64_t mlinference::CudaLib::deviceUlong(nvid_ctx *ctx, DeviceProperty property) noexcept
{
    return pDeviceUlong(ctx, property);
}


void mlinference::CudaLib::release(nvid_ctx *ctx) noexcept
{
    pRelease(ctx);
}


bool mlinference::CudaLib::open()
{
    m_error = nullptr;

    if (custom_uv_dlopen(m_loader, &cudaLib) == 0) {
        return true;
    }
...
#   endif
    ...
    m_error = custom_uv_dlerror(&cudaLib);

    return false;
}


void mlinference::CudaLib::load()
{
    DLSYM(Version);
