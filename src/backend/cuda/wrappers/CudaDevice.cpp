...
mlinference::CudaDevice::CudaDevice(uint32_t index, int32_t bfactor, int32_t bsleep) :
    m_index(index)
{
...
mlinference::CudaDevice::CudaDevice(CudaDevice &&other) noexcept :
...
mlinference::CudaDevice::~CudaDevice()
{
    CudaLib::release(m_ctx);
}


size_t mlinference::CudaDevice::freeMemSize() const
{
    return CudaLib::deviceUlong(m_ctx, CudaLib::DeviceMemoryFree);
}


size_t mlinference::CudaDevice::globalMemSize() const
{
    return CudaLib::deviceUlong(m_ctx, CudaLib::DeviceMemoryTotal);
}


uint32_t mlinference::CudaDevice::clock() const
{
    return CudaLib::deviceUint(m_ctx, CudaLib::DeviceClockRate) / 1000;
}


uint32_t mlinference::CudaDevice::computeCapability(bool major) const
{
    return CudaLib::deviceUint(m_ctx, major ? CudaLib::DeviceArchMajor : CudaLib::DeviceArchMinor);
}


uint32_t mlinference::CudaDevice::memoryClock() const
{
    return CudaLib::deviceUint(m_ctx, CudaLib::DeviceMemoryClockRate) / 1000;
}


uint32_t mlinference::CudaDevice::smx() const
{
    return CudaLib::deviceUint(m_ctx, CudaLib::DeviceSmx);
}


void mlinference::CudaDevice::generate(const Algorithm &algorithm, CudaThreads &threads) const
{
...
void mlinference::CudaDevice::toJSON(rapidjson::Value &out, rapidjson::Document &doc) const
{
