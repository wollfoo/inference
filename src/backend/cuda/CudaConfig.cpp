...
rapidjson::Value mlinference::CudaConfig::toJSON(rapidjson::Document &doc) const
{
...
std::vector<mlinference::CudaLaunchData> mlinference::CudaConfig::get(const Miner *miner, const Algorithm &algorithm, const std::vector<CudaDevice> &devices) const
{
...
void mlinference::CudaConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
void mlinference::CudaConfig::generate()
{
    if (generated) {
        return;
    }
...
void mlinference::CudaConfig::setDevicesHint(const char *devicesHint)
{
