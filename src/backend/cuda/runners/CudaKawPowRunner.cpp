...
mlinference::CudaKawPowRunner::CudaKawPowRunner(size_t index, const CudaLaunchData &data) :
    CudaBaseRunner(index, data)
{
}


bool mlinference::CudaKawPowRunner::run(uint32_t /*startNonce*/, uint32_t *rescount, uint32_t *resnonce)
{
    return callWrapper(CudaLib::kawPowHash(m_ctx, m_jobBlob, m_target, rescount, resnonce, &m_skippedHashes));
}


bool mlinference::CudaKawPowRunner::set(const Job &job, uint8_t *blob)
{
...
void mlinference::CudaKawPowRunner::jobEarlyNotification(const Job&)
{
