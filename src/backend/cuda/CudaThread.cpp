...
mlinference::CudaThread::CudaThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
mlinference::CudaThread::CudaThread(uint32_t index, nvid_ctx *ctx) :
...
bool mlinference::CudaThread::isEqual(const CudaThread &other) const
{
...
rapidjson::Value mlinference::CudaThread::toJSON(rapidjson::Document &doc) const
{
