...
mlinference::CudaWorker::CudaWorker(size_t id, const CudaLaunchData &data) :
...
mlinference::CudaWorker::~CudaWorker()
{
    delete m_runner;
}


void mlinference::CudaWorker::jobEarlyNotification(const Job &job)
{
    if (m_runner) {
...
bool mlinference::CudaWorker::selfTest()
{
    return m_runner != nullptr;
}


size_t mlinference::CudaWorker::intensity() const
{
    return m_runner ? m_runner->roundSize() : 0;
}


void mlinference::CudaWorker::start()
{
...
bool mlinference::CudaWorker::consumeJob()
{
    if (Nonce::sequence(Nonce::CUDA) == 0) {
        return false;
    }
...
void mlinference::CudaWorker::storeStats()
{
    if (!isReady()) {
        return;
    }
