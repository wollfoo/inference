...
namespace mlinference { static const char *kNvmlLabel = "NVML"; }
#endif
...
const char *mlinference::cuda_tag()
{
    return Tags::nvidia();
}


mlinference::CudaBackend::CudaBackend(Controller *controller) :
...
mlinference::CudaBackend::~CudaBackend()
{
    delete d_ptr;
...
bool mlinference::CudaBackend::isEnabled() const
{
    return d_ptr->controller->config()->cuda().isEnabled() && CudaLib::isInitialized() && !d_ptr->devices.empty();;
}


bool mlinference::CudaBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cuda().threads().get(algorithm).isEmpty();
}


const mlinference::Hashrate *mlinference::CudaBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const mlinference::String &mlinference::CudaBackend::profileName() const
{
    return d_ptr->profileName;
}


const mlinference::String &mlinference::CudaBackend::type() const
{
    return kType;
}


void mlinference::CudaBackend::execCommand(char)
{
}


void mlinference::CudaBackend::prepare(const Job &job)
{
    if (d_ptr) {
...
void mlinference::CudaBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void mlinference::CudaBackend::printHealth()
{
#   ifdef XMRIG_FEATURE_NVML
    d_ptr->printHealth();
#   endif
...
void mlinference::CudaBackend::setJob(const Job &job)
{
...
void mlinference::CudaBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void mlinference::CudaBackend::stop()
{
...
bool mlinference::CudaBackend::tick(uint64_t ticks)
{
...
#ifdef XMRIG_FEATURE_API
rapidjson::Value mlinference::CudaBackend::toJSON(rapidjson::Document &doc) const
{
...
void mlinference::CudaBackend::handleRequest(IApiRequest &)
{
}
#endif
