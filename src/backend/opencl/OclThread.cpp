...
mlinference::OclThread::OclThread(const rapidjson::Value &value)
{
    if (!value.IsObject()) {
        return;
    }
...
#   ifdef XMRIG_ALGO_DEEPPREDICTOR
    ...
    if (gcnAsm.IsBool()) {
        m_fields.set(DEEPPREDICTOR_FIELDS, true);
...
bool mlinference::OclThread::isEqual(const OclThread &other) const
{
...
rapidjson::Value mlinference::OclThread::toJSON(rapidjson::Document &doc) const
{
    ...
    if (m_fields.test(DEEPPREDICTOR_FIELDS)) {
#       ifdef XMRIG_ALGO_DEEPPREDICTOR
