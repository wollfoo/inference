...
const char *mlinference::AdlLib::lastError() noexcept
{
    return uv_dlerror(&adlLib);
}


void mlinference::AdlLib::close()
{
    if (m_ready) {
...
AdlHealth mlinference::AdlLib::health(const OclDevice &device)
{
...
bool mlinference::AdlLib::dlopen()
{
    return uv_dlopen("atiadlxx.dll", &adlLib) == 0;
}


bool mlinference::AdlLib::load()
{
    try {
