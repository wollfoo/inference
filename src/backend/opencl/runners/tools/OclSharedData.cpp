...
cl_mem mlinference::OclSharedData::createBuffer(cl_context context, size_t size, size_t &offset, size_t limit)
{
...
uint64_t mlinference::OclSharedData::adjustDelay(size_t /*id*/)
{
    if (m_threads < 2) {
        return 0;
    }
...
uint64_t mlinference::OclSharedData::resumeDelay(size_t /*id*/)
{
    if (m_threads < 2) {
        return 0;
    }
...
void mlinference::OclSharedData::release()
{
...
void mlinference::OclSharedData::setResumeCounter(uint32_t value)
{
    if (m_threads < 2) {
        return;
    }
...
void mlinference::OclSharedData::setRunTime(uint64_t time)
{
...
#ifdef XMRIG_ALGO_DEEPPREDICTOR
cl_mem mlinference::OclSharedData::dataset() const
{
    if (!m_dataset) {
        throw std::runtime_error("DeeppredictoR dataset is not available");
    }
...
void mlinference::OclSharedData::createDataset(cl_context ctx, const Job &job, bool host)
{
    if (m_dataset) {
        return;
    }
