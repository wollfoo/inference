...
#include "backend/opencl/cl/rx/deeppredictor_run_gfx803.h"
#include "backend/opencl/cl/rx/deeppredictor_run_gfx900.h"
#include "backend/opencl/cl/rx/deeppredictor_run_gfx1010.h"
...
mlinference::OclRxJitRunner::OclRxJitRunner(size_t index, const OclLaunchData &data) : OclRxBaseRunner(index, data)
{
}


mlinference::OclRxJitRunner::~OclRxJitRunner()
{
    delete m_deeppredictor_jit;
    delete m_deeppredictor_run;
...
void mlinference::OclRxJitRunner::build()
{
    ...
    m_deeppredictor_jit = new RxJitKernel(m_program);
    m_deeppredictor_jit->setArgs(m_entropy, m_registers, m_intermediate_programs, m_programs, m_intensity, m_rounding);

    if (!loadAsmProgram()) {
        throw std::runtime_error(OclError::toString(CL_INVALID_PROGRAM));
    }

    m_deeppredictor_run = new RxRunKernel(m_asmProgram);
    m_deeppredictor_run->setArgs(m_dataset, m_scratchpads, m_registers, m_rounding, m_programs, m_intensity, m_algorithm);
}


void mlinference::OclRxJitRunner::execute(uint32_t iteration)
{
    m_deeppredictor_jit->enqueue(m_queue, m_intensity, iteration);

    OclLib::finish(m_queue);

    m_deeppredictor_run->enqueue(m_queue, m_intensity, (m_gcn_version == 15) ? 32 : 64);
}


void mlinference::OclRxJitRunner::init()
{
...
bool mlinference::OclRxJitRunner::loadAsmProgram()
{
    ...
    case 14:
        len = deeppredictor_run_gfx900_bin_size;
        binary = deeppredictor_run_gfx900_bin;
        break;
    case 15:
        len = deeppredictor_run_gfx1010_bin_size;
        binary = deeppredictor_run_gfx1010_bin;
        break;
    default:
        len = deeppredictor_run_gfx803_bin_size;
        binary = deeppredictor_run_gfx803_bin;
        break;
    }
