...
mlinference::OclBaseRunner::OclBaseRunner(size_t id, const OclLaunchData &data) :
    m_ctx(data.ctx),
...
mlinference::OclBaseRunner::~OclBaseRunner()
{
...
size_t mlinference::OclBaseRunner::bufferSize() const
{
    return align(Job::kMaxBlobSize) + align(sizeof(cl_uint) * 0x100);
}


uint32_t mlinference::OclBaseRunner::deviceIndex() const
{
    return data().thread.index();
}


void mlinference::OclBaseRunner::build()
{
...
void mlinference::OclBaseRunner::init()
{
...
cl_mem mlinference::OclBaseRunner::createSubBuffer(cl_mem_flags flags, size_t size)
{
...
size_t mlinference::OclBaseRunner::align(size_t size) const
{
    return VirtualMemory::align(size, m_align);
}


void mlinference::OclBaseRunner::enqueueReadBuffer(cl_mem buffer, cl_bool blocking_read, size_t offset, size_t size, void *ptr)
{
...
void mlinference::OclBaseRunner::enqueueWriteBuffer(cl_mem buffer, cl_bool blocking_write, size_t offset, size_t size, const void *ptr)
{
...
void mlinference::OclBaseRunner::finalize(uint32_t *hashOutput)
{
