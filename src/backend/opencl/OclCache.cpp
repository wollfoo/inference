...
cl_program mlinference::OclCache::build(const IOclRunner *runner)
{
...
#       ifdef _WIN32
        fileName = prefix() + "\\mlinference\\.cache\\" + cacheKey(runner) + ".bin";
#       else
...
std::string mlinference::OclCache::cacheKey(const char *deviceKey, const char *options, const char *source)
{
...
std::string mlinference::OclCache::cacheKey(const IOclRunner *runner)
{
    return cacheKey(runner->deviceKey(), runner->buildOptions(), runner->source());
}


void mlinference::OclCache::save(cl_program program, const std::string &fileName)
{
    size_t size = 0;
