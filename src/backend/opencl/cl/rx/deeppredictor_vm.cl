...
#define ScratchpadL3Mask64 (DEEPPREDICTOR_SCRATCHPAD_L3 - CacheLineSize)
#define CacheLineAlignMask ((DEEPPREDICTOR_DATASET_BASE_SIZE - 1) & ~(CacheLineSize - 1))
...
#define ConditionMask ((1 << DEEPPREDICTOR_JUMP_BITS) - 1)
#define ConditionOffset DEEPPREDICTOR_JUMP_OFFSET
#define StoreL3Condition 14
#define DatasetExtraItems (DEEPPREDICTOR_DATASET_EXTRA_SIZE / DEEPPREDICTOR_DATASET_ITEM_SIZE)
...
{
#if DEEPPREDICTOR_PROGRAM_SIZE <= 256
	typedef uint8_t exec_t;
#else
	typedef uint16_t exec_t;
#endif

	__local uint32_t execution_plan_buf[DEEPPREDICTOR_PROGRAM_SIZE * WORKERS_PER_HASH * (32 / 8) * sizeof(exec_t) / sizeof(uint32_t)];
	...
	__local exec_t* execution_plan = (__local exec_t*)(execution_plan_buf + (get_local_id(0) / 8) * DEEPPREDICTOR_PROGRAM_SIZE * WORKERS_PER_HASH * sizeof(exec_t) / sizeof(uint32_t));
	...
	if (sub == 0)
	{
		...
		for (uint32_t i = 0; i < DEEPPREDICTOR_PROGRAM_SIZE; ++i)
		{
			...
			*(__global uint32_t*)(src_program + i) &= ~(0xF8U << 8);
			...
			if (opcode < DEEPPREDICTOR_FREQ_IADD_RS + DEEPPREDICTOR_FREQ_IADD_M + DEEPPREDICTOR_FREQ_ISUB_R + DEEPPREDICTOR_FREQ_ISUB_M + DEEPPREDICTOR_FREQ_IMUL_R + DEEPPREDICTOR_FREQ_IMUL_M + DEEPPREDICTOR_FREQ_IMULH_R + DEEPPREDICTOR_FREQ_IMULH_M + DEEPPREDICTOR_FREQ_ISMULH_R + DEEPPREDICTOR_FREQ_ISMULH_M)
			{
#if DEEPPREDICTOR_PROGRAM_SIZE <= 256
			...
			opcode -= DEEPPREDICTOR_FREQ_IADD_RS + DEEPPREDICTOR_FREQ_IADD_M + DEEPPREDICTOR_FREQ_ISUB_R + DEEPPREDICTOR_FREQ_ISUB_M + DEEPPREDICTOR_FREQ_IMUL_R + DEEPPREDICTOR_FREQ_IMUL_M + DEEPPREDICTOR_FREQ_IMULH_R + DEEPPREDICTOR_FREQ_IMULH_M + DEEPPREDICTOR_FREQ_ISMULH_R + DEEPPREDICTOR_FREQ_ISMULH_M;

			if (opcode < DEEPPREDICTOR_FREQ_IMUL_RCP)
			{
				if (inst.y & (inst.y - 1))
				{
#if DEEPPREDICTOR_PROGRAM_SIZE <= 256
			...
			opcode -= DEEPPREDICTOR_FREQ_IMUL_RCP;

			if (opcode < DEEPPREDICTOR_FREQ_INEG_R + DEEPPREDICTOR_FREQ_IXOR_R + DEEPPREDICTOR_FREQ_IXOR_M + DEEPPREDICTOR_FREQ_IROR_R + DEEPPREDICTOR_FREQ_IROL_R)
			{
#if DEEPPREDICTOR_PROGRAM_SIZE <= 256
				set_byte(registerLastChanged, dst, i);
				set_byte(registerWasChanged, dst, 1);
			...
			opcode -= DEEPPREDICTOR_FREQ_INEG_R + DEEPPREDICTOR_FREQ_IXOR_R + DEEPPREDICTOR_FREQ_IXOR_M + DEEPPREDICTOR_FREQ_IROR_R + DEEPPREDICTOR_FREQ_IROL_R;

			if (opcode < DEEPPREDICTOR_FREQ_ISWAP_R)
			{
				if (src != dst)
				{
#if DEEPPREDICTOR_PROGRAM_SIZE <= 256
			...
			opcode -= DEEPPREDICTOR_FREQ_ISWAP_R;

			if (opcode < DEEPPREDICTOR_FREQ_FSWAP_R + DEEPPREDICTOR_FREQ_FADD_R + DEEPPREDICTOR_FREQ_FADD_M + DEEPPREDICTOR_FREQ_FSUB_R + DEEPPREDICTOR_FREQ_FSUB_M + DEEPPREDICTOR_FREQ_FSCAL_R + DEEPPREDICTOR_FREQ_FMUL_R + DEEPPREDICTOR_FREQ_FDIV_M + DEEPPREDICTOR_FREQ_FSQRT_R)
			{
				// Mark FP instruction (src |= 0x20)
				*(__global uint32_t*)(src_program + i) |= 0x20 << 8;
				continue;
			}
			opcode -= DEEPPREDICTOR_FREQ_FSWAP_R + DEEPPREDICTOR_FREQ_FADD_R + DEEPPREDICTOR_FREQ_FADD_M + DEEPPREDICTOR_FREQ_FSUB_R + DEEPPREDICTOR_FREQ_FSUB_M + DEEPPREDICTOR_FREQ_FSCAL_R + DEEPPREDICTOR_FREQ_FMUL_R + DEEPPREDICTOR_FREQ_FDIV_M + DEEPPREDICTOR_FREQ_FSQRT_R;

			if (opcode < DEEPPREDICTOR_FREQ_CBRANCH)
			{
		...
		//if (global_index == 0)
		//{
		//	for (int j = 0; j < DEEPPREDICTOR_PROGRAM_SIZE; ++j)
		//	{
		...
		for (uint32_t i = 0; i < DEEPPREDICTOR_PROGRAM_SIZE; ++i)
		{
			...
			const uint32_t mem_read_latency = ((dst == src) && ((inst.y & ScratchpadL3Mask64) >= DEEPPREDICTOR_SCRATCHPAD_L2)) ? ScratchpadHighLatency : ScratchpadLatency;
			...
			do {
				if (opcode < DEEPPREDICTOR_FREQ_IADD_RS)
				{
					latency = reg_read_latency;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_IADD_RS;

				if (opcode < DEEPPREDICTOR_FREQ_IADD_M)
				{
					latency = full_read_latency;
					is_memory_op = true;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_IADD_M;

				if (opcode < DEEPPREDICTOR_FREQ_ISUB_R)
				{
					latency = reg_read_latency;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_ISUB_R;

				if (opcode < DEEPPREDICTOR_FREQ_ISUB_M)
				{
					latency = full_read_latency;
					is_memory_op = true;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_ISUB_M;

				if (opcode < DEEPPREDICTOR_FREQ_IMUL_R)
				{
					latency = reg_read_latency;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_IMUL_R;

				if (opcode < DEEPPREDICTOR_FREQ_IMUL_M)
				{
					latency = full_read_latency;
					is_memory_op = true;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_IMUL_M;

				if (opcode < DEEPPREDICTOR_FREQ_IMULH_R)
				{
					latency = reg_read_latency;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_IMULH_R;

				if (opcode < DEEPPREDICTOR_FREQ_IMULH_M)
				{
					latency = full_read_latency;
					is_memory_op = true;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_IMULH_M;

				if (opcode < DEEPPREDICTOR_FREQ_ISMULH_R)
				{
					latency = reg_read_latency;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_ISMULH_R;

				if (opcode < DEEPPREDICTOR_FREQ_ISMULH_M)
				{
					latency = full_read_latency;
					is_memory_op = true;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_ISMULH_M;
				...
				if (opcode < DEEPPREDICTOR_FREQ_INEG_R)
				{
					is_src_read = false;
					latency = dst_latency;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_INEG_R;

				if (opcode < DEEPPREDICTOR_FREQ_IXOR_R)
				{
					latency = reg_read_latency;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_IXOR_R;

				if (opcode < DEEPPREDICTOR_FREQ_IXOR_M)
				{
					latency = full_read_latency;
					is_memory_op = true;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_IXOR_M;

				if (opcode < DEEPPREDICTOR_FREQ_IROR_R + DEEPPREDICTOR_FREQ_IROL_R)
				{
					latency = reg_read_latency;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_IROR_R + DEEPPREDICTOR_FREQ_IROL_R;
				...
				if (opcode < DEEPPREDICTOR_FREQ_FSWAP_R)
				{
				...
				opcode -= DEEPPREDICTOR_FREQ_FSWAP_R;

				if (opcode < DEEPPREDICTOR_FREQ_FADD_R)
				{
				...
				opcode -= DEEPPREDICTOR_FREQ_FADD_R;

				if (opcode < DEEPPREDICTOR_FREQ_FADD_M)
				{
					dst %= RegisterCountFlt;
					latency = get_byte(registerLatencyFP, dst);
				...
				opcode -= DEEPPREDICTOR_FREQ_FADD_M;

				if (opcode < DEEPPREDICTOR_FREQ_FSUB_R)
				{
					dst %= RegisterCountFlt;
					latency = get_byte(registerLatencyFP, dst);
					is_fp = true;
				...
				opcode -= DEEPPREDICTOR_FREQ_FSUB_R;

				if (opcode < DEEPPREDICTOR_FREQ_FSUB_M)
				{
					dst %= RegisterCountFlt;
					latency = get_byte(registerLatencyFP, dst);
					update_max(latency, src_latency);
				...
				opcode -= DEEPPREDICTOR_FREQ_FSUB_M;

				if (opcode < DEEPPREDICTOR_FREQ_FSCAL_R)
				{
					dst %= RegisterCountFlt;
					latency = get_byte(registerLatencyFP, dst);
					is_fp = true;
				...
				opcode -= DEEPPREDICTOR_FREQ_FSCAL_R;

				if (opcode < DEEPPREDICTOR_FREQ_FMUL_R)
				{
				...
				opcode -= DEEPPREDICTOR_FREQ_FMUL_R;

				if (opcode < DEEPPREDICTOR_FREQ_FDIV_M)
				{
					dst = (dst % RegisterCountFlt) + RegisterCountFlt;
					latency = get_byte(registerLatencyFP, dst);
					update_max(latency, src_latency);
				...
				opcode -= DEEPPREDICTOR_FREQ_FDIV_M;

				if (opcode < DEEPPREDICTOR_FREQ_FSQRT_R)
				{
					dst = (dst % RegisterCountFlt) + RegisterCountFlt;
					latency = get_byte(registerLatencyFP, dst);
					is_fp = true;
				...
				opcode -= DEEPPREDICTOR_FREQ_FSQRT_R;
				...
				opcode -= DEEPPREDICTOR_FREQ_CBRANCH;

				if (opcode < DEEPPREDICTOR_FREQ_CFROUND)
				{
					latency = src_latency;
					is_cfround = true;
					break;
				}
				opcode -= DEEPPREDICTOR_FREQ_CFROUND;

				if (opcode < DEEPPREDICTOR_FREQ_ISTORE)
				{
					latency = reg_read_latency;
				...
				opcode -= DEEPPREDICTOR_FREQ_ISTORE;
			...
			if (execution_plan[first_available_slot] || (first_available_slot == first_instruction_slot))
			{
				...
				do {
					++first_available_slot;
				} while ((first_available_slot < DEEPPREDICTOR_PROGRAM_SIZE * WORKERS_PER_HASH) && (execution_plan[first_available_slot] != 0));
			}
		...
		for (int32_t i = 0; i <= last_used_slot; ++i)
		{
			...
			if (opcode < DEEPPREDICTOR_FREQ_IROR_R + DEEPPREDICTOR_FREQ_IROL_R)
			{
				...
				if (opcode >= DEEPPREDICTOR_FREQ_IROR_R)
				{
...
#if DEEPPREDICTOR_FREQ_IROL_R > 0
...
__kernel void execute_vm(__global void* vm_states, __global void* rounding, __global void* scratchpads, __global const void* dataset_ptr, uint32_t batch_size, uint32_t num_iterations, uint32_t first, uint32_t last)
{
	...
	__global uint8_t* scratchpad = ((__global uint8_t*)scratchpads) + idx * (uint64_t)(DEEPPREDICTOR_SCRATCHPAD_L3 + 64);
	...
	for (int ic = 0; ic < num_iterations; ++ic)
	{
		...
		//if ((global_index == 0) && (ic == DEEPPREDICTOR_PROGRAM_ITERATIONS - 1))
		//{
		//	printf("ic = %d (after)\n", ic);
		//	for (int i = 0; i < 8; ++i)
		//		printf("r%d = %016llx\n", i, R[i]);
