...
mlinference::OclWorker::OclWorker(size_t id, const OclLaunchData &data) :
...
mlinference::OclWorker::~OclWorker()
{
    delete m_runner;
}


void mlinference::OclWorker::jobEarlyNotification(const Job &job)
{
    if (m_runner) {
...
bool mlinference::OclWorker::selfTest()
{
    return m_runner != nullptr;
}


size_t mlinference::OclWorker::intensity() const
{
    return m_runner ? m_runner->roundSize() : 0;
}


void mlinference::OclWorker::start()
{
...
bool mlinference::OclWorker::consumeJob()
{
    if (Nonce::sequence(Nonce::OPENCL) == 0) {
        return false;
    }
...
void mlinference::OclWorker::storeStats(uint64_t t)
{
    if (!isReady()) {
        return;
    }
