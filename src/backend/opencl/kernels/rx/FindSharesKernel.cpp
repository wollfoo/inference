...
void mlinference::FindSharesKernel::enqueue(cl_command_queue queue, size_t threads)
{
...
void mlinference::FindSharesKernel::setArgs(cl_mem hashes, cl_mem shares)
{
...
void mlinference::FindSharesKernel::setTarget(uint64_t target)
{
    setArg(1, sizeof(uint64_t), &target);
}


void mlinference::FindSharesKernel::setNonce(uint32_t nonce)
{
