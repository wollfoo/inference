...
#ifndef XMRIG_OS_APPLE
mlinference::OclConfig::OclConfig() : m_platformVendor(kAMD) {}
#else
mlinference::OclConfig::OclConfig() = default;
#endif


mlinference::OclPlatform mlinference::OclConfig::platform() const
{
...
rapidjson::Value mlinference::OclConfig::toJSON(rapidjson::Document &doc) const
{
...
std::vector<mlinference::OclLaunchData> mlinference::OclConfig::get(const Miner *miner, const Algorithm &algorithm, const OclPlatform &platform, const std::vector<OclDevice> &devices) const
{
...
void mlinference::OclConfig::read(const rapidjson::Value &value)
{
    if (value.IsObject()) {
...
void mlinference::OclConfig::generate()
{
    ...
    count += mlinference::generate<Algorithm::CN>(m_threads, devices);
    count += mlinference::generate<Algorithm::CN_LITE>(m_threads, devices);
    count += mlinference::generate<Algorithm::CN_HEAVY>(m_threads, devices);
    count += mlinference::generate<Algorithm::CN_PICO>(m_threads, devices);
    count += mlinference::generate<Algorithm::CN_FEMTO>(m_threads, devices);
    count += mlinference::generate<Algorithm::DEEPPREDICTO_R>(m_threads, devices);
    count += mlinference::generate<Algorithm::KAWPOW>(m_threads, devices);

    m_shouldSave = count > 0;
}


void mlinference::OclConfig::setDevicesHint(const char *devicesHint)
{
...
#ifndef XMRIG_OS_APPLE
void mlinference::OclConfig::setPlatform(const rapidjson::Value &platform)
{
