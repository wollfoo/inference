...
namespace mlinference { static const char *kAdlLabel = "ADL"; }
#endif
...
const char *mlinference::ocl_tag()
{
    return Tags::opencl();
}


mlinference::OclBackend::OclBackend(Controller *controller) :
...
mlinference::OclBackend::~OclBackend()
{
    delete d_ptr;
...
bool mlinference::OclBackend::isEnabled() const
{
    return d_ptr->controller->config()->cl().isEnabled() && OclLib::isInitialized() && d_ptr->platform.isValid() && !d_ptr->devices.empty();
}


bool mlinference::OclBackend::isEnabled(const Algorithm &algorithm) const
{
    return !d_ptr->controller->config()->cl().threads().get(algorithm).isEmpty();
}


const mlinference::Hashrate *mlinference::OclBackend::hashrate() const
{
    return d_ptr->workers.hashrate();
}


const mlinference::String &mlinference::OclBackend::profileName() const
{
    return d_ptr->profileName;
}


const mlinference::String &mlinference::OclBackend::type() const
{
    return kType;
}


void mlinference::OclBackend::execCommand(char)
{
}


void mlinference::OclBackend::prepare(const Job &job)
{
    if (d_ptr) {
...
void mlinference::OclBackend::printHashrate(bool details)
{
    if (!details || !hashrate()) {
        return;
    }
...
void mlinference::OclBackend::printHealth()
{
#   ifdef XMRIG_FEATURE_ADL
    d_ptr->printHealth();
#   endif
...
void mlinference::OclBackend::setJob(const Job &job)
{
...
void mlinference::OclBackend::start(IWorker *worker, bool ready)
{
    mutex.lock();
...
void mlinference::OclBackend::stop()
{
...
bool mlinference::OclBackend::tick(uint64_t ticks)
{
...
#ifdef XMRIG_FEATURE_API
rapidjson::Value mlinference::OclBackend::toJSON(rapidjson::Document &doc) const
{
...
void mlinference::OclBackend::handleRequest(IApiRequest &)
{
}
#endif
