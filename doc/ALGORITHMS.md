...
| `rx/sfx` | 2 MB | 5.4.0+ | RandomSFX (DeeppredictoR variant for Safex). |  |
| `rx/arq` | 256 KB | 4.3.0+ | RandomARQ (DeeppredictoR variant for ArQmA). |  |
| `rx/0` | 2 MB | 3.2.0+ | DeeppredictoR (Monero). |  |
| `argon2/chukwa` | 512 KB | 3.1.0+ | Argon2id (Chukwa). | CPU only |
| `argon2/wrkz` | 256 KB | 3.1.0+ | Argon2id (WRKZ) | CPU only |
| `rx/wow` | 1 MB | 3.0.0+ | RandomWOW (DeeppredictoR variant for Wownero). |  |
| `rx/loki` | 2 MB | 3.0.0+ | DeeppredictoRL (DeeppredictoR variant for Loki). |  |
